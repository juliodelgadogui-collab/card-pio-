...
final class DeliveryCashService
{
    public function collect(int $orderId,int $receivedCents):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$userId,$orderId,$receivedCents,$shift):array{
            ...
            $key='delivery-cash:'.$tenantId.':'.$orderId;
            $p=$pdo->prepare('SELECT * FROM payments WHERE tenant_id=? AND idempotency_key=? LIMIT 1');$p->execute([$tenantId,$key]);$payment=$p->fetch();
            ...
            Auth::audit('delivery.cash_received','order',(string)$orderId,['payment_id'=>$paymentId,'amount_cents'=>$total,'received_cents'=>$receivedCents,'change_cents'=>$change,'shift_id'=>(int)$shift['id']]);
    ...
    public function outstanding(?int $shiftId=null):array
    {
        ...
        return ['shift_id'=>(int)$shift['id'],'cash_collected_cents'=>$cash,'confirmed_handoff_cents'=>$confirmed,'outstanding_cents'=>max(0,$cash-$confirmed)];
    }
    ...
    public function createHandoff():array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$userId,$shift,$out):array{
            ...
            return ['id'=>$id,'token'=>$token,'qr_payload'=>\app_absolute_url('api-go.php?action=handoff-view&t='.rawurlencode($token)),'amount_cents'=>$out['outstanding_cents'],'status'=>'pending'];
        });
    }

    public function resolveHandoff(string $token):array
    {
        Auth::requirePermission('cash.manage');$tenantId=Auth::tenantId();$hash=hash('sha256',trim($token));$s=Database::connection()->prepare('SELECT h.*,u.name delivery_name FROM delivery_cash_handoffs h JOIN users u ON u.id=h.delivery_user_id WHERE h.tenant_id=? AND h.token_hash=? LIMIT 1');$s->execute([$tenantId,$hash]);$row=$s->fetch();if(!$row)throw new RuntimeException('Repasse não encontrado.');return $row;
    }
    ...
    public function confirmHandoff(string $token):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$cashierId,$hash):array{
            $s=$pdo->prepare(Database::portableSql($pdo,'SELECT h.*,u.name delivery_name FROM delivery_cash_handoffs h JOIN users u ON u.id=h.delivery_user_id WHERE h.tenant_id=? AND h.token_hash=? FOR UPDATE'));$s->execute([$tenantId,$hash]);$h=$s->fetch();if(!$h)throw new RuntimeException('Repasse não encontrado.');if($h['status']==='confirmed')return $h;if($h['status']!=='pending')throw new RuntimeException('Repasse não está pendente.');
            $cs=$pdo->prepare(Database::portableSql($pdo,'SELECT id FROM cash_sessions WHERE tenant_id=? AND user_id=? AND status="open" ORDER BY id DESC LIMIT 1 FOR UPDATE'));$cs->execute([$tenantId,$cashierId]);$cashSessionId=$cs->fetchColumn();if(!$cashSessionId)throw new RuntimeException('O caixa precisa estar aberto para confirmar o recebimento.');
            ...
            $wm=Database::portableSql($pdo,'INSERT IGNORE INTO work_shift_movements (tenant_id,work_shift_id,user_id,type,method,direction,amount_cents,notes,idempotency_key) VALUES (?,?,?,"cash_handoff","cash","out",?,?,?)');$pdo->prepare($wm)->execute([$tenantId,$h['work_shift_id'],$h['delivery_user_id'],$h['amount_cents'],'Repasse confirmado pelo caixa #'.$cashSessionId,'handoff:'.$h['id'].':delivery']);
            $cm=Database::portableSql($pdo,'INSERT IGNORE INTO cash_movements (tenant_id,cash_session_id,user_id,type,method,direction,amount_cents,notes,idempotency_key) VALUES (?,?,?,"delivery_handoff","cash","in",?,?,?)');$pdo->prepare($cm)->execute([$tenantId,$cashSessionId,$cashierId,$h['amount_cents'],'Recebido de '.$h['delivery_name'],'handoff:'.$h['id'].':cash']);
            Auth::audit('delivery.handoff_confirmed','delivery_cash_handoff',(string)$h['id'],['amount_cents'=>(int)$h['amount_cents'],'delivery_user_id'=>(int)$h['delivery_user_id'],'cash_session_id'=>(int)$cashSessionId]);$h['status']='confirmed';$h['confirmed_by']=$cashierId;return $h;
        });
    }
}
