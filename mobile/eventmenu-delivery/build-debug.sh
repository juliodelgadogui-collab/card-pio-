...
CACHE_DIR="${HOME}/.eventmenu-delivery"
...
if [[ ! -x "${GRADLE_HOME_DIR}/bin/gradle" ]]; then
  ...
  if command -v curl >/dev/null 2>&1; then curl -fL "${DIST_URL}" -o "${ZIP_FILE}"
  elif command -v wget >/dev/null 2>&1; then wget -O "${ZIP_FILE}" "${DIST_URL}"
  else echo "Erro: instale curl ou wget para baixar o Gradle." >&2; exit 1; fi
...
if [[ ! -f local.properties ]]; then
  ...
  if [[ -n "${SDK_PATH}" ]]; then printf 'sdk.dir=%s\n' "${SDK_PATH}" > local.properties; fi
fi
