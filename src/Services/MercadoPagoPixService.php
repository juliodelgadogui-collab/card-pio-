...
final class MercadoPagoPixService
{
    public function create(int $orderId,string $taxId='',?int $amountCents=null):array
    {
        ...
        $key='pix:mercadopago:'.$tenantId.':'.$orderId.':'.$paidBefore.':'.$amount;
        $existing=$pdo->prepare('SELECT * FROM payments WHERE tenant_id=? AND order_id=? AND provider="mercadopago" AND idempotency_key=? LIMIT 1');$existing->execute([$tenantId,$orderId,$key]);$payment=$existing->fetch();
        ...
        // payer.identification must identify the payer/customer. Never substitute
        // receipt_document or pix_default_document from the establishment.
        $doc=preg_replace('/\D+/','',$taxId)??'';
        if($doc==='')$doc=preg_replace('/\D+/','',(string)($order['customer_document']??''))??'';
        $doc=$this->validTaxId($doc);
    ...
    private function validTaxId(string $value):string
    {
        ...
        if(!in_array(strlen($v),[11,14],true)||preg_match('/^(\d)\1+$/',$v))throw new RuntimeException('Informe CPF ou CNPJ válido do pagador para gerar o PIX.');
        if(strlen($v)===11){for($t=9;$t<11;$t++){$sum=0;for($i=0;$i<$t;$i++)$sum+=(int)$v[$i]*(($t+1)-$i);$d=(10*($sum%11))%11;if($d===10)$d=0;if((int)$v[$t]!==$d)throw new RuntimeException('CPF do pagador inválido.');}return$v;}
        $calc=static function(string $base,array $weights):int{$sum=0;foreach($weights as$i=>$w)$sum+=(int)$base[$i]*$w;$r=$sum%11;return$r<2?0:11-$r;};$d1=$calc(substr($v,0,12),[5,4,3,2,9,8,7,6,5,4,3,2]);$d2=$calc(substr($v,0,12).$d1,[6,5,4,3,2,9,8,7,6,5,4,3,2]);if((int)$v[12]!==$d1||(int)$v[13]!==$d2)throw new RuntimeException('CNPJ do pagador inválido.');return$v;
    }
