...
final class OrderCreationService
{
    public function create(array $payload):array
    {
        ...
        $order=Database::transaction(function(PDO $tx)use($tenantId,$userId,$unitId,$channel,$tableId,$eventId,$name,$phone,$address,$notes,$quantities):array{
            ...
            $token=bin2hex(random_bytes(20));$o=$tx->prepare('INSERT INTO orders (public_token,tenant_id,unit_id,customer_id,channel,status,payment_status,subtotal_cents,total_cents,table_id,tab_id,event_id,delivery_address,notes,created_by) VALUES (?,?,?,?,?, ?,"unpaid",?,?,?,?,?,?,?,?)');$o->execute([$token,$tenantId,$effectiveUnitId,$customerId,$channel,$initialStatus,$total,$total,$validTableId,$tabId,$validEventId,$channel==='delivery'?$address:null,$notes?:null,$userId]);$orderId=(int)$tx->lastInsertId();
            ...
            Auth::audit('order.created_staff','order',(string)$orderId,['channel'=>$channel,'event_id'=>$validEventId,'unit_id'=>$effectiveUnitId,'total_cents'=>$total,'items'=>count($items)]);
            return ['id'=>$orderId,'public_token'=>$token,'channel'=>$channel,'status'=>$initialStatus,'payment_status'=>'unpaid','total_cents'=>$total,'customer_id'=>$customerId,'table_id'=>$validTableId,'tab_id'=>$tabId,'event_id'=>$validEventId,'unit_id'=>$effectiveUnitId];
        });
