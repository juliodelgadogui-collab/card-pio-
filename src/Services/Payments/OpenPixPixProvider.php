...
final class OpenPixPixProvider implements PixProviderInterface
{
    ...
    {
        $config=$gateway['config'];$appId=trim((string)($config['app_id']??$config['token']??''));$id=trim((string)($payment['provider_payment_id']??''));if($appId===''||$id==='')throw new RuntimeException('Cobrança OpenPix não está pronta para consulta.');$data=$this->requestJson('OpenPix','GET','https://api.openpix.com.br/api/v1/charge/'.rawurlencode($id),['Authorization: '.$appId]);$charge=is_array($data['charge']??null)?$data['charge']:$data;$status=strtoupper((string)($charge['status']??''));$amount=(int)($charge['value']??$charge['valueWithDiscount']??0);return['paid'=>$status==='COMPLETED','provider_payment_id'=>(string)($charge['correlationID']??$id),'amount_cents'=>$amount,'currency'=>'BRL','account_reference'=>(string)$gateway['account_reference'],'raw_status'=>$status,'raw'=>$data];
    }
}
