...
final class PurchaseService
{
    ...
    {
        Auth::requirePermission('inventory.manage');$tenantId=Auth::tenantId();$userId=Auth::id();if(!$tenantId||!$userId)throw new RuntimeException('Sessão inválida.');
        Database::transaction(function(PDO$pdo)use($tenantId,$userId,$orderId):void{
            $order=$this->lockedOrder($pdo,$tenantId,$orderId);$this->assertCurrentUnit($order);if($order['status']==='received')return;if(!in_array($order['status'],['draft','ordered'],true))throw new RuntimeException('Esta compra não pode ser recebida.');
            $items=$pdo->prepare('SELECT poi.*,p.name,p.track_stock,p.average_cost_cents,p.min_stock_qty FROM purchase_order_items poi JOIN products p ON p.id=poi.product_id WHERE poi.purchase_order_id=? ORDER BY poi.id');$items->execute([$orderId]);$rows=$items->fetchAll();if(!$rows)throw new RuntimeException('Compra sem itens.');$unitId=(int)$order['unit_id'];
            foreach($rows as$row){if(!(int)$row['track_stock'])throw new RuntimeException('O produto '.$row['name'].' está sem controle de estoque.');$productId=(int)$row['product_id'];$factor=max(.000001,(float)($row['purchase_factor_snapshot']??1));$stockQty=(float)($row['stock_quantity']??0);if($stockQty<=0)$stockQty=(float)$row['quantity']*$factor;if($stockQty<=0)throw new RuntimeException('Quantidade convertida inválida para '.$row['name'].'.');$insert=Database::portableSql($pdo,'INSERT IGNORE INTO unit_inventory (tenant_id,unit_id,product_id,stock_qty,average_cost_cents,min_stock_qty) VALUES (?,?,?,?,?,?)');$pdo->prepare($insert)->execute([$tenantId,$unitId,$productId,0,$row['average_cost_cents']??0,$row['min_stock_qty']??0]);$inv=$pdo->prepare(Database::portableSql($pdo,'SELECT * FROM unit_inventory WHERE tenant_id=? AND unit_id=? AND product_id=? FOR UPDATE'));$inv->execute([$tenantId,$unitId,$productId]);$inventory=$inv->fetch();if(!$inventory)throw new RuntimeException('Saldo da unidade não encontrado para '.$row['name'].'.');$reserved=$pdo->prepare('SELECT COALESCE(SUM(quantity),0) FROM stock_reservations WHERE tenant_id=? AND unit_id=? AND product_id=? AND status="reserved"');$reserved->execute([$tenantId,$unitId,$productId]);$physical=max(0,(float)$inventory['stock_qty']+(float)$reserved->fetchColumn());$stockUnitCost=(int)round((int)$row['total_cents']/$stockQty);$den=$physical+$stockQty;$average=$den>0?(int)round((($physical*(int)$inventory['average_cost_cents'])+($stockQty*$stockUnitCost))/$den):$stockUnitCost;$pdo->prepare('UPDATE unit_inventory SET stock_qty=stock_qty+?,average_cost_cents=?,updated_at=CURRENT_TIMESTAMP WHERE tenant_id=? AND unit_id=? AND product_id=?')->execute([$stockQty,$average,$tenantId,$unitId,$productId]);$key='purchase:'.$orderId.':item:'.$row['id'];$exists=$pdo->prepare('SELECT id FROM stock_movements WHERE tenant_id=? AND idempotency_key=?');$exists->execute([$tenantId,$key]);if(!$exists->fetchColumn())$pdo->prepare('INSERT INTO stock_movements (tenant_id,unit_id,product_id,order_id,type,quantity,idempotency_key,unit_cost_cents,total_cost_cents,reason,performed_by) VALUES (?,?,?,NULL,"in",?,?,?,?,?,?)')->execute([$tenantId,$unitId,$productId,$stockQty,$key,$stockUnitCost,(int)$row['total_cents'],'Recebimento da compra #'.$orderId,$userId]);$this->syncLegacyProduct($pdo,$tenantId,$productId);}
            $pdo->prepare('UPDATE purchase_orders SET status="received",received_by=?,received_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?')->execute([$userId,$orderId,$tenantId]);
            (new PurchasePayableService())->generateForOrderInTransaction($pdo,$tenantId,$orderId);
        });
        Auth::audit('purchase.received','purchase_order',(string)$orderId);
    }
