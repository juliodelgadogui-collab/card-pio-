...
final class NfcService
{
    public function createIntent(int $orderId,string $deviceIdentifier,?int $amountCents=null):array
    {
        ...
        $shift=(new WorkShiftService())->current();if($shift&&$shift['mode']==='delivery'){if(!Auth::can('orders.delivery')||(int)($order['assigned_delivery_user_id']??0)!==$userId)throw new RuntimeException('Este pedido não está atribuído a você.');(new DeliveryProgressService())->assertArrived($orderId);}
    ...
    public function verifyIntent(string $intentToken,string $transactionCode,string $deviceIdentifier):array
    {
        ...
        $shift=(new WorkShiftService())->current();if($shift&&$shift['mode']==='delivery'){if((int)($intent['assigned_delivery_user_id']??0)!==$userId)throw new RuntimeException('A entrega não está mais atribuída a você.');(new DeliveryProgressService())->assertArrived((int)$intent['order_id']);}
