...
// Compatibilidade com carrinhos antigos: produto_id => quantidade.
$legacy=false;
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(!Security::validateCsrf($_POST['_csrf']??null))$error='Sua sessão expirou. Atualize a página e tente novamente.';
        ...
        if($action==='add'){
            try{
                $id=(int)($_POST['product_id']??0);$qty=max(1,min(99,(int)($_POST['qty']??1)));
            ...
            try{
                $order=(new PublicMenuService())->create(
                    $tenantId,array_values($cart),(string)($_POST['name']??''),(string)($_POST['phone']??''),(string)($_POST['address']??''),
                    $table?$tableToken:null,$table?'table':$selectedFulfillment
                );
                $cart=[];
        ...
        if($action!=='checkout'&&$error===null){
            $query=['empresa'=>$slug];if($table)$query['mesa']=$tableToken;
...
$p=$pdo->prepare('SELECT p.*,EXISTS(SELECT 1 FROM product_recipes r WHERE r.tenant_id=p.tenant_id AND r.product_id=p.id) has_recipe FROM products p WHERE p.tenant_id=? AND p.active=1 ORDER BY p.category_id,p.name');$p->execute([$tenantId]);$products=$p->fetchAll();
...
$productMap=[];foreach($products as$prod)$productMap[(int)$prod['id']]=$prod;

$cartRows=[];$cartTotal=0;$cartCount=0;$invalid=[];
foreach($cart as$lineKey=>$line){
    try{
        $resolved=$configured->resolveLine($pdo,$tenantId,$line,false);$mods=[];
        foreach($resolved['modifiers'] as$m)$mods[]=$m['group_name'].': '.$m['name'];
        $cartTotal+=(int)$resolved['total_cents'];$cartCount+=(int)ceil((float)$resolved['quantity']);
    ...
    }catch(Throwable$e){$invalid[]=$lineKey;if($error===null)$error='Um item do carrinho foi alterado e precisa ser adicionado novamente.';}
}
...
@media(max-width:480px){.topbar{padding-top:3px}.contact a{font-size:0}.contact a:after{content:'Contato';font-size:10px}.identity strong{font-size:16px}.identity small{font-size:9px}.hero-cover{height:130px}.fulfillment{grid-template-columns:1fr}.product{grid-template-columns:82px minmax(0,1fr)}.photo{width:82px;height:82px}.add-row input[type=number]{display:none}}
</style></head><body><main class="shell">
<header class="topbar"><div class="identity"><?php if($logoUrl!==''):?><img class="logo" src="<?= Security::e($logoUrl) ?>" alt="Logo <?= Security::e($title) ?>"><?php else:?><div class="logo monogram"><?= Security::e(mb_strtoupper(mb_substr($title,0,1))) ?></div><?php endif;?><div><strong><?= Security::e($title) ?></strong><small><?= $table?'Pedido pela mesa':'Cardápio digital' ?></small></div></div><div class="contact"><?php if($whatsapp!==''):?><a target="_blank" rel="noopener" href="https://wa.me/<?= Security::e($whatsapp) ?>">WhatsApp</a><?php endif;?></div></header>
<section class="hero <?= Security::e($headerStyle) ?><?= $coverUrl!==''?' with-cover':'' ?>"><?php if($coverUrl!==''):?><img class="hero-cover" src="<?= Security::e($coverUrl) ?>" alt=""><?php endif;?><div class="hero-body"><span class="eyebrow"><?= $table?'PEDIDO NA MESA':'CARDÁPIO DIGITAL' ?></span><h1><?= Security::e($title) ?></h1><p><?= Security::e($subtitle) ?></p><div class="meta"><?php if($table):?><span class="pill open"><?= Security::e($table['name']) ?></span><span class="pill">Comanda automática</span><?php else:?><span class="pill open">Aceitando pedidos</span><span class="pill">Retirada</span><span class="pill">Delivery</span><?php if($deliveryFee>0):?><span class="pill">Entrega <?= menu_money($deliveryFee) ?></span><?php endif;?><?php endif;?></div><?php if(!empty($settings['menu_message'])):?><div class="message"><?= Security::e((string)$settings['menu_message']) ?></div><?php endif;?></div></section>
