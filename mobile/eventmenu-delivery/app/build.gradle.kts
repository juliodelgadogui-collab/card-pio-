...
// Debug builds generated by CI must use the exact cached key selected by the
// workflow. Android Gradle Plugin otherwise may resolve a different default
// debug.keystore, which makes successive APKs incompatible for in-place updates.
val debugKeystorePath = envValue("DELYVRE_DEBUG_KEYSTORE_PATH")
val debugStorePassword = envRaw("DELYVRE_DEBUG_STORE_PASSWORD").ifBlank { "android" }
val debugKeyAlias = envValue("DELYVRE_DEBUG_KEY_ALIAS").ifBlank { "androiddebugkey" }
val debugKeyPassword = envRaw("DELYVRE_DEBUG_KEY_PASSWORD").ifBlank { "android" }
if (debugKeystorePath.isNotBlank() && !file(debugKeystorePath).isFile) {
    throw GradleException("Keystore debug do DELYVRE não encontrado em DELYVRE_DEBUG_KEYSTORE_PATH.")
}
...
android {
    ...
    signingConfigs {
        if (debugKeystorePath.isNotBlank()) {
            getByName("debug") {
