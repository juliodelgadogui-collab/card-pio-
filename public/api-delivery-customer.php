...
try{
    ...
    if($action==='register'){if($_SERVER['REQUEST_METHOD']!=='POST')delivery_customer_out(['ok'=>false,'message'=>'Ação indisponível.'],405);$rate->assertAllowed('delivery.customer.register',$subject,5,3600,'Muitas tentativas de cadastro. Aguarde e tente novamente.');$result=Database::transaction(fn(PDO$tx):array=>$auth->register($tx,$body));delivery_customer_out(['ok'=>true]+$result,201);}
