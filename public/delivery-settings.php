...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        $pixEnabled=!empty($_POST['mp_pix_enabled']);$cardEnabled=!empty($_POST['mp_card_enabled']);$activeRequested=!empty($_POST['mp_active']);
        ...
        $mp['access_token']=$access;$mp['public_key']=$public;$mp['pix_enabled']=$pixEnabled;$mp['card_enabled']=$cardEnabled;$mp['max_installments']=max(1,min(12,(int)($_POST['mp_max_installments']??12)));
        ...
        Auth::audit('delivery.settings_saved','tenant',(string)$tenantId,['delivery_paused'=>$settings['delivery_paused'],'cash'=>$settings['delivery_cash_enabled'],'pix'=>$mp['pix_enabled'],'card'=>$mp['card_enabled'],'mercadopago_account'=>$account!==''?'configured':'missing','webhook_secret'=>$hasWebhookSecret?'configured':'missing']);
