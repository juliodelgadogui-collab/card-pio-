...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'plan-save') {
        ...
        try {
            if ($id > 0) {
                ...
                $s->execute([$code,$name,$description,$monthly,$yearly,$active,$sortOrder,json_encode($features,JSON_UNESCAPED_UNICODE),json_encode($limits,JSON_UNESCAPED_UNICODE)]);$id=(int)$pdo->lastInsertId();
            }
            Auth::audit('platform.plan_saved','saas_plan',(string)$id,['code'=>$code,'monthly_cents'=>$monthly,'yearly_cents'=>$yearly]);em_flash('ok','Plano comercial salvo.');
    ...
    if ($action === 'plan-toggle') {
        ...
        $id=(int)($_POST['id']??0);$active=(int)($_POST['new_active']??0)===1?1:0;$pdo->prepare('UPDATE saas_plans SET active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')->execute([$active,$id]);Auth::audit('platform.plan_status','saas_plan',(string)$id,['active'=>$active]);em_flash('ok',$active?'Plano reativado.':'Plano arquivado.');em_go('super',['tab'=>'plans']);
    }

    if ($action === 'tenant-status') {
        $tenantId=(int)($_POST['tenant_id']??0);$status=(string)($_POST['status']??'');if($tenantId<1||!in_array($status,['active','suspended','cancelled'],true))exit('Alteração inválida.');
        try {
            Database::transaction(function(PDO $tx)use($tenantId,$status,&$tenant):void{
                $s=$tx->prepare(Database::portableSql($tx,'SELECT id,name,status FROM tenants WHERE id=? FOR UPDATE'));$s->execute([$tenantId]);$tenant=$s->fetch();if(!$tenant)throw new RuntimeException('Empresa não encontrada.');$tx->prepare('UPDATE tenants SET status=? WHERE id=?')->execute([$status,$tenantId]);
                if($status!=='active'){$tx->prepare('UPDATE api_tokens SET revoked_at=CURRENT_TIMESTAMP WHERE tenant_id=? AND revoked_at IS NULL')->execute([$tenantId]);try{$tx->prepare('UPDATE api_refresh_tokens SET revoked_at=CURRENT_TIMESTAMP WHERE tenant_id=? AND revoked_at IS NULL')->execute([$tenantId]);}catch(Throwable){}$tx->prepare('UPDATE nfc_devices SET status="revoked",revoked_at=CURRENT_TIMESTAMP WHERE tenant_id=? AND status<>"revoked"')->execute([$tenantId]);$tx->prepare('UPDATE nfc_payment_intents SET status="failed" WHERE tenant_id=? AND status="created"')->execute([$tenantId]);$tx->prepare('UPDATE work_shifts SET status="closed",ended_at=CURRENT_TIMESTAMP,closing_notes="Empresa suspensa/cancelada" WHERE tenant_id=? AND status="open"')->execute([$tenantId]);}
            });
            if(Auth::actingTenantId()===$tenantId&&$status!=='active')Auth::clearTenantContext();Auth::audit('platform.tenant_status','tenant',(string)$tenantId,['from'=>$tenant['status'],'to'=>$status,'name'=>$tenant['name']]);em_flash('ok',$status==='active'?'Empresa reativada.':'Empresa atualizada e acessos operacionais revogados.');
    ...
    if ($action === 'tenant-plan') {
        ...
        $tenantId=(int)($_POST['tenant_id']??0);$planId=(int)($_POST['plan_id']??0);$cycle=(string)($_POST['billing_cycle']??'monthly');if(!in_array($cycle,['monthly','yearly','custom'],true))$cycle='monthly';$customPrice=$cycle==='custom'?$moneyToCents($_POST['custom_price']??0):null;
        try{
            Database::transaction(function(PDO $tx)use($tenantId,$planId,$cycle,$customPrice,&$plan):void{$p=$tx->prepare('SELECT id,code,name FROM saas_plans WHERE id=?');$p->execute([$planId]);$plan=$p->fetch();if(!$plan)throw new RuntimeException('Plano não encontrado.');$t=$tx->prepare('SELECT id FROM tenants WHERE id=?');$t->execute([$tenantId]);if(!$t->fetchColumn())throw new RuntimeException('Empresa não encontrada.');$tx->prepare('UPDATE tenants SET plan=? WHERE id=?')->execute([$plan['code'],$tenantId]);$exists=$tx->prepare('SELECT id FROM tenant_subscriptions WHERE tenant_id=?');$exists->execute([$tenantId]);if($exists->fetchColumn())$tx->prepare('UPDATE tenant_subscriptions SET plan_id=?,status="active",billing_cycle=?,custom_price_cents=?,ends_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE tenant_id=?')->execute([$planId,$cycle,$customPrice,$tenantId]);else$tx->prepare('INSERT INTO tenant_subscriptions (tenant_id,plan_id,status,billing_cycle,custom_price_cents) VALUES (?,? ,"active",?,?)')->execute([$tenantId,$planId,$cycle,$customPrice]);});
            Auth::audit('platform.tenant_plan','tenant',(string)$tenantId,['plan'=>$plan['code'],'billing_cycle'=>$cycle,'custom_price_cents'=>$customPrice]);em_flash('ok','Plano da empresa atualizado para '.$plan['name'].'.');
        }catch(Throwable $e){em_flash('error',$e->getMessage());}
        em_go('super',['tab'=>'companies']);
    }

    if ($action === 'tenant-create') {
        $tenantName=mb_substr(trim((string)($_POST['tenant_name']??'')),0,160);$slug=em_slug((string)($_POST['slug']??$tenantName));$businessType=(string)($_POST['business_type']??'full');
        $adminName=mb_substr(trim((string)($_POST['admin_name']??'')),0,160);$adminEmail=mb_strtolower(trim((string)($_POST['admin_email']??'')));$adminPhone=mb_substr(trim((string)($_POST['admin_phone']??'')),0,40);$adminPassword=(string)($_POST['admin_password']??'');
        ...
        try{
            $plan=null;if($plansReady){$ps=$pdo->prepare('SELECT id,code FROM saas_plans WHERE id=? AND active=1');$ps->execute([(int)($_POST['plan_id']??0)]);$plan=$ps->fetch()?:null;}$planCode=(string)($plan['code']??'premium');$planId=(int)($plan['id']??0);
            $created=Database::transaction(function(PDO $tx)use($tenantName,$slug,$planCode,$planId,$businessType,$plansReady,$adminName,$adminEmail,$adminPhone,$adminPassword):array{
                ...
                $settings=json_encode(['business_type'=>$businessType,'points_enabled'=>true,'menu_primary_color'=>'#6236df','menu_background_color'=>'#f7f7fb','menu_surface_color'=>'#ffffff','menu_text_color'=>'#252137','menu_layout'=>'cards','menu_header_style'=>'gradient','menu_show_images'=>true,'menu_show_search'=>true,'menu_show_branding'=>true],JSON_UNESCAPED_UNICODE);
            ...
            Auth::audit('platform.tenant_created','tenant',(string)$created['tenant_id'],['slug'=>$created['slug'],'plan'=>$planCode,'business_type'=>$businessType,'admin_user_id'=>$created['user_id']]);Auth::actAsTenant((int)$created['tenant_id']);em_flash('ok','Empresa e administrador criados. O cliente já pode entrar com '.$adminEmail.'.');em_go('dashboard');
        }catch(Throwable $e){em_flash('error',$e->getMessage());em_go('super',['tab'=>'companies']);}
    }
}

$plans=[];if($plansReady)$plans=$pdo->query('SELECT * FROM saas_plans ORDER BY active DESC,sort_order,name')->fetchAll();$activePlans=array_values(array_filter($plans,static fn(array$p):bool=>(int)$p['active']===1));
$stats=['tenants'=>(int)$pdo->query('SELECT COUNT(*) FROM tenants')->fetchColumn(),'active'=>(int)$pdo->query('SELECT COUNT(*) FROM tenants WHERE status="active"')->fetchColumn(),'users'=>(int)$pdo->query('SELECT COUNT(*) FROM users WHERE role<>"super_admin"')->fetchColumn(),'sales'=>(int)$pdo->query('SELECT COALESCE(SUM(total_cents),0) FROM orders WHERE payment_status="paid"')->fetchColumn(),'mrr'=>0];
...
$q=trim((string)($_GET['q']??''));$sql='SELECT t.*,(SELECT COUNT(*) FROM users u WHERE u.tenant_id=t.id) users_count,(SELECT name FROM users u WHERE u.tenant_id=t.id AND u.role="admin" ORDER BY u.id LIMIT 1) admin_name,(SELECT email FROM users u WHERE u.tenant_id=t.id AND u.role="admin" ORDER BY u.id LIMIT 1) admin_email,(SELECT COUNT(*) FROM orders o WHERE o.tenant_id=t.id) orders_count,(SELECT COALESCE(SUM(o.total_cents),0) FROM orders o WHERE o.tenant_id=t.id AND o.payment_status="paid") paid_total';if($plansReady)$sql.=',ts.plan_id subscription_plan_id,ts.billing_cycle,ts.custom_price_cents,sp.name subscription_plan_name';$sql.=' FROM tenants t';if($plansReady)$sql.=' LEFT JOIN tenant_subscriptions ts ON ts.tenant_id=t.id LEFT JOIN saas_plans sp ON sp.id=ts.plan_id';$args=[];if($q!==''){$sql.=' WHERE t.name LIKE ? OR t.slug LIKE ? OR EXISTS (SELECT 1 FROM users ux WHERE ux.tenant_id=t.id AND (ux.name LIKE ? OR ux.email LIKE ?))';$like='%'.$q.'%';$args=[$like,$like,$like,$like];}$sql.=' ORDER BY t.id DESC LIMIT 300';$s=$pdo->prepare($sql);$s->execute($args);$tenants=$s->fetchAll();
$editPlanId=(int)($_GET['edit_plan']??0);$editPlan=null;foreach($plans as$p)if((int)$p['id']===$editPlanId)$editPlan=$p;$editFeatures=$editPlan?(json_decode((string)($editPlan['features_json']??'[]'),true)?:[]):[];$editLimits=$editPlan?(json_decode((string)($editPlan['limits_json']??'{}'),true)?:[]):[];$tab=(string)($_GET['tab']??'overview');if(!in_array($tab,['overview','plans','companies'],true))$tab='overview';$statusLabels=['active'=>'Ativa','suspended'=>'Suspensa','cancelled'=>'Cancelada'];

em_header('Gestão da plataforma','super');
?>
<section class="page-hero platform-hero"><div><span class="eyebrow">EVENTMENU SAAS</span><h2>Controle comercial da plataforma</h2><p>Gerencie clientes, planos, acessos e operação em um único painel.</p></div><div class="hero-actions"><a class="button primary" href="<?= Security::e(app_url('?route=super&tab=companies#nova-empresa')) ?>">+ Nova empresa</a><a class="button secondary" href="<?= Security::e(app_url('?route=super&tab=plans')) ?>">Gerenciar planos</a></div></section>
