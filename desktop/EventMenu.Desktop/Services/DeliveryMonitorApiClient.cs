...
public sealed class DeliveryMonitorApiClient : IDisposable
{
    ...
    public DeliveryMonitorApiClient(SecureSessionStore store)
    {
        _store = store;
    ...
    {
        try { return JsonSerializer.Deserialize<ApiError>(text, JsonOptions)?.Error ?? "Não foi possível atualizar as entregas."; }
        catch { return "Não foi possível atualizar as entregas."; }
    }
