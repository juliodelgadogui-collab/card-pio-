...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='save'){
        ...
        if($id){
            $u=$pdo->prepare('SELECT id FROM users WHERE id=? AND tenant_id=?');
            $u->execute([$id,$tenantId]);
            if(!$u->fetchColumn())exit('Usuário não encontrado.');
            if($password!==''&&strlen($password)<10)exit('A senha precisa ter pelo menos 10 caracteres.');
            if($password!==''){
            ...
            if(strlen($password)<10)exit('A senha precisa ter pelo menos 10 caracteres.');
        ...
        if($status==='blocked'||!in_array($role,$paymentRoles,true)){
            $revoke=$pdo->prepare('UPDATE nfc_devices SET status="revoked",revoked_at=NOW() WHERE tenant_id=? AND user_id=? AND status<>"revoked"');
            $revoke->execute([$tenantId,$id]);
            if($revoke->rowCount()>0)Auth::audit('nfc.revoked_by_user_change','user',(string)$id,['role'=>$role,'status'=>$status,'devices'=>$revoke->rowCount()]);
        }
