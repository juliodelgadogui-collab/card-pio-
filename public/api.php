...
try{
 if($action==='login'){api_method('POST');$body=api_body();$result=$auth->login((string)($body['email']??''),(string)($body['password']??''),(string)($body['device_id']??''),(string)($body['device_label']??''));api_out(['ok'=>true]+$result);}
 if($action==='refresh'){api_method('POST');$body=api_body();$deviceId=ApiAuthService::deviceId();if($deviceId==='')$deviceId=(string)($body['device_id']??'');$result=$auth->refresh((string)($body['refresh_token']??''),$deviceId);api_out(['ok'=>true]+$result);}
 ...
 if($action==='me')api_out(['ok'=>true,'user'=>$user,'permissions'=>['orders_create'=>Auth::can('orders.create'),'orders_manage'=>Auth::can('orders.manage'),'orders_kitchen'=>Auth::can('orders.kitchen'),'orders_delivery'=>Auth::can('orders.delivery'),'delivery_assign'=>Auth::can('delivery.assign'),'cash'=>Auth::can('cash.manage'),'nfc_collect'=>Auth::can('nfc.collect'),'tickets'=>Auth::can('tickets.manage'),'guests'=>Auth::can('guests.manage'),'tables'=>Auth::can('tables.manage')]]);
 if($action==='customer-lookup'){
  if(!Auth::can('orders.create'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$phone=(string)($_GET['phone']??'');$customer=(new CustomerIdentityService())->findByPhone($pdo,$tenantId,$phone);api_out(['ok'=>true,'found'=>$customer!==null,'customer'=>$customer]);
 }
 if($action==='customer-save'){
  api_method('POST');if(!Auth::can('orders.create'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$body=api_body();$id=(int)($body['customer_id']??0)?:null;$customer=(new CustomerIdentityService())->save($pdo,$tenantId,['name'=>$body['name']??'','phone'=>$body['phone']??'','address'=>$body['address']??'','email'=>$body['email']??''],$id);Auth::audit($id?'customer.updated':'customer.created','customer',(string)$customer['id'],['source'=>'api-go']);api_out(['ok'=>true,'customer'=>$customer],$id?200:201);
 }
 if($action==='products'){if(!Auth::can('orders.create')&&!Auth::can('catalog.manage'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$s=$pdo->prepare('SELECT id,category_id,name,description,sku,price_cents,stock_qty,track_stock,image_url FROM products WHERE tenant_id=? AND active=1 ORDER BY name');$s->execute([$tenantId]);api_out(['ok'=>true,'products'=>$s->fetchAll()]);}
 if($action==='order-create'){api_method('POST');if(!Auth::can('orders.create'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$order=(new OrderCreationService())->create(api_body());api_out(['ok'=>true,'order'=>$order],201);}
 if($action==='orders'){
  $role=Auth::role();$sql='SELECT o.id,o.public_token,o.channel,o.status,o.payment_status,o.total_cents,o.delivery_address,o.assigned_delivery_user_id,o.table_id,o.tab_id,o.created_at,c.name customer_name,c.phone customer_phone,u.name delivery_name FROM orders o LEFT JOIN customers c ON c.id=o.customer_id LEFT JOIN users u ON u.id=o.assigned_delivery_user_id WHERE o.tenant_id=?';$args=[$tenantId];if($role==='delivery'){$sql.=' AND o.assigned_delivery_user_id=? AND o.status IN ("ready","out_for_delivery")';$args[]=Auth::id();}elseif($role==='kitchen'){$sql.=' AND o.status IN ("confirmed","preparing")';}elseif(!Auth::can('orders.view')&&!Auth::can('orders.create'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$sql.=' ORDER BY o.id DESC LIMIT 200';$s=$pdo->prepare($sql);$s->execute($args);api_out(['ok'=>true,'orders'=>$s->fetchAll()]);
 }
 if($action==='order'){$id=(int)($_GET['id']??0);$s=$pdo->prepare('SELECT o.*,c.name customer_name,c.phone customer_phone,u.name delivery_name FROM orders o LEFT JOIN customers c ON c.id=o.customer_id LEFT JOIN users u ON u.id=o.assigned_delivery_user_id WHERE o.id=? AND o.tenant_id=?');$s->execute([$id,$tenantId]);$order=$s->fetch();if(!$order)api_out(['ok'=>false,'error'=>'Pedido não encontrado.'],404);if(!api_order_allowed($order))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$i=$pdo->prepare('SELECT id,product_id,name_snapshot,unit_price_cents,quantity,total_cents,notes FROM order_items WHERE order_id=? ORDER BY id');$i->execute([$id]);api_out(['ok'=>true,'order'=>$order,'items'=>$i->fetchAll()]);}
 if($action==='order-status'){api_method('POST');$body=api_body();$id=(int)($body['order_id']??0);$status=(string)($body['status']??'');$s=$pdo->prepare('SELECT * FROM orders WHERE id=? AND tenant_id=?');$s->execute([$id,$tenantId]);$order=$s->fetch();if(!$order)api_out(['ok'=>false,'error'=>'Pedido não encontrado.'],404);if(!api_order_allowed($order))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$role=Auth::role();if($role==='delivery')$source='delivery';elseif($role==='kitchen')$source='kitchen';else{if(!Auth::can('orders.manage'))api_out(['ok'=>false,'error'=>'Sua função pode visualizar/criar pedidos, mas não alterar o status operacional.'],403);$source='panel';}$changed=(new OrderService())->changeStatus($id,$status,$source);api_out(['ok'=>true,'order'=>$changed]);}
 if($action==='delivery-assign'){api_method('POST');if(!Auth::can('delivery.assign'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$body=api_body();$orderId=(int)($body['order_id']??0);$deliveryId=(int)($body['delivery_user_id']??0);Database::transaction(function(PDO $tx)use($tenantId,$orderId,$deliveryId):void{$o=$tx->prepare(Database::portableSql($tx,'SELECT id,channel,status FROM orders WHERE id=? AND tenant_id=? FOR UPDATE'));$o->execute([$orderId,$tenantId]);$order=$o->fetch();if(!$order)throw new RuntimeException('Pedido não encontrado.');if($order['channel']!=='delivery')throw new RuntimeException('Somente pedidos de delivery podem ser atribuídos.');if(in_array($order['status'],['completed','cancelled'],true))throw new RuntimeException('Pedido já encerrado.');if($deliveryId){$d=$tx->prepare('SELECT id FROM users WHERE id=? AND tenant_id=? AND role="delivery" AND status="active"');$d->execute([$deliveryId,$tenantId]);if(!$d->fetchColumn())throw new RuntimeException('Entregador inválido.');}$tx->prepare('UPDATE orders SET assigned_delivery_user_id=? WHERE id=? AND tenant_id=?')->execute([$deliveryId?:null,$orderId,$tenantId]);});Auth::audit('order.delivery_assigned','order',(string)$orderId,['delivery_user_id'=>$deliveryId?:null,'source'=>'api']);api_out(['ok'=>true]);}
 if($action==='delivery-users'){if(!Auth::can('delivery.assign'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$s=$pdo->prepare('SELECT id,name,phone FROM users WHERE tenant_id=? AND role="delivery" AND status="active" ORDER BY name');$s->execute([$tenantId]);api_out(['ok'=>true,'delivery_users'=>$s->fetchAll()]);}
 // Keep the remaining specialized actions in api-go.php; this compatibility API handles core POS/order operations.
 api_out(['ok'=>false,'error'=>'Ação não encontrada.'],404);
}catch(Throwable $e){api_out(['ok'=>false,'error'=>$e->getMessage()!==''?$e->getMessage():'Não foi possível concluir a operação.'],400);}
