...
final class MarketplaceCatalogService
{
    ...
    public function catalog(PDO $pdo, int $tenantId, int $unitId): array
    {
        ...
        $tenant = $pdo->prepare("SELECT id,name,slug,settings FROM tenants WHERE id=? AND status='active' LIMIT 1");
        $tenant->execute([$tenantId]);
        $row = $tenant->fetch();
        if (!$row) throw new RuntimeException('Loja indisponível.');

        $unit = $pdo->prepare('SELECT id,code,name,address FROM operating_units WHERE id=? AND tenant_id=? AND active=1 LIMIT 1');
        $unit->execute([$unitId, $tenantId]);
        $unitRow = $unit->fetch();
        if (!$unitRow) throw new RuntimeException('Unidade indisponível.');

        $settings = json_decode((string)($row['settings'] ?? '{}'), true) ?: [];

        $c = $pdo->prepare('SELECT id,name,sort_order FROM categories WHERE tenant_id=? AND active=1 ORDER BY sort_order,name');
        $c->execute([$tenantId]);
        ...
        foreach ($products as $product) {
            ...
            foreach ($modifiers[(int)$product['id']] ?? [] as $group) {
                ...
                $min = max((int)($group['min_select'] ?? 0), (int)($group['required'] ?? 0) ? 1 : 0);
                $max = max(1, (int)($group['max_select'] ?? 1));
                $groups[] = [
                    'id' => (int)$group['id'],
            ...
            $safeProducts[] = [
                ...
                'image_url' => $this->publicUrl($product['image_url'] ?? ''),
                'available' => $available,
