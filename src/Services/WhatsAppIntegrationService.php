...
final class WhatsAppIntegrationService
{
    ...
    {
        $limit=max(1,min(500,$limit));$sql='SELECT o.tenant_id,o.id order_id,MAX(p.id) payment_id FROM orders o JOIN payments p ON p.tenant_id=o.tenant_id AND p.order_id=o.id JOIN whatsapp_connections wc ON wc.tenant_id=o.tenant_id WHERE wc.automation_enabled=1 AND wc.automation_enabled_at IS NOT NULL AND o.payment_status="paid" AND p.status="paid" AND COALESCE(p.verified_at,p.created_at)>=wc.automation_enabled_at GROUP BY o.tenant_id,o.id ORDER BY payment_id DESC LIMIT '.$limit;$rows=$pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC)?:[];$queued=0;foreach($rows as$row){try{if($this->queueOrderEvent($pdo,(int)$row['tenant_id'],(int)$row['order_id'],'payment_confirmed','payment:'.(int)$row['payment_id'])!==null)$queued++;}catch(Throwable){}}return$queued;
    }
