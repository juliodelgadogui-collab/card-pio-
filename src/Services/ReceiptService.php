...
final class ReceiptService
{
    public function order(int $orderId): array
    {
        ...
        $tickets=[];$event=null;try{$t=$pdo->prepare('SELECT tk.id,tk.code,tk.qr_token,tk.status,e.id event_id,e.name event_name,e.starts_at,e.ends_at,e.venue,e.address,b.name batch_name,tt.name ticket_type_name FROM tickets tk JOIN events e ON e.id=tk.event_id AND e.tenant_id=tk.tenant_id JOIN ticket_batches b ON b.id=tk.batch_id AND b.event_id=tk.event_id LEFT JOIN ticket_types tt ON tt.id=b.ticket_type_id AND tt.tenant_id=tk.tenant_id AND tt.event_id=tk.event_id WHERE tk.tenant_id=? AND tk.order_id=? ORDER BY tk.id');$t->execute([$tenantId,$orderId]);$tickets=$t->fetchAll();}catch(\Throwable){try{$t=$pdo->prepare('SELECT tk.id,tk.code,tk.qr_token,tk.status,e.id event_id,e.name event_name,e.starts_at,e.ends_at,e.venue,e.address,b.name batch_name,NULL ticket_type_name FROM tickets tk JOIN events e ON e.id=tk.event_id AND e.tenant_id=tk.tenant_id JOIN ticket_batches b ON b.id=tk.batch_id AND b.event_id=tk.event_id WHERE tk.tenant_id=? AND tk.order_id=? ORDER BY tk.id');$t->execute([$tenantId,$orderId]);$tickets=$t->fetchAll();}catch(\Throwable){$tickets=[];}}
