...
function em_nav():array{return array_filter([Auth::role()==='super_admin'?['superadmin','Super ADM','superadmin.only']:null,['dashboard','Visão geral','dashboard'],['pos','PDV / Balcão','orders.create'],['fulfillment','Retiradas','fulfillment.manage'],['products','Cardápio','catalog.manage'],['orders','Pedidos','orders.view'],['delivery','Delivery','delivery.assign'],['kitchen','Cozinha / KDS','orders.kitchen'],['restaurant','Mesas e comandas','tables.manage'],['cash','Caixa','payments.manage'],['customers','Clientes e pontos','customers.manage'],['coupons','Cupons','coupons.manage'],['events','Eventos','events.manage'],['tickets','Ingressos / Check-in','tickets.manage'],['guests','Convidados','guests.manage'],['promoters','Promotores','promoters.manage'],['payments','Pagamentos','payments.manage'],['gateways','Gateways e NFC','gateways.manage'],['users','Equipe','users.manage'],['reports','Relatórios','reports.any'],['audit','Auditoria','audit.view'],['settings','Configurações','settings.manage']]);}
function em_header(string $title,string $active):void{$flash=em_flash();?><!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#0b0f14"><title><?= Security::e($title) ?> — EventMenu Premium</title><link rel="manifest" href="/manifest.webmanifest"><link rel="stylesheet" href="/assets/app.css"></head><body><div class="layout"><aside class="sidebar"><div class="brand">EventMenu <span>Premium</span></div><nav class="nav"><?php foreach(em_nav() as$item):[$route,$label,$permission]=$item;if(!em_can_nav($permission))continue;?><a class="<?= $active===$route?'active':'' ?>" href="/?route=<?= Security::e($route) ?>"><?= Security::e($label) ?></a><?php endforeach;?></nav><div class="sidebar-foot"><strong><?= Security::e(Auth::name()) ?></strong><br><span><?= Security::e((string)Auth::role()) ?></span><?php if(Auth::role()==='super_admin'):?><br><span class="muted"><?= Auth::tenantId()?'Empresa #'.(int)Auth::tenantId():'Contexto global' ?></span><?php endif;?><br><br><a href="/?route=logout">Sair</a></div></aside><main class="content"><header class="topbar"><div><h1><?= Security::e($title) ?></h1><div class="muted">EventMenu Premium · operação multiempresa</div></div><?php if(Auth::can('users.manage') && Auth::tenantId()):?><a class="button secondary" href="/update.php">Atualizar banco</a><?php endif;?></header><?php if($flash):?><div class="alert <?= Security::e($flash[0]) ?>"><?= Security::e($flash[1]) ?></div><?php endif;?><?php }
