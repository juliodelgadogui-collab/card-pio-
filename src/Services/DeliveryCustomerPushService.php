...
final class DeliveryCustomerPushService
{
    ...
    private function accessToken():string
    {
        ...
        $c=$this->credentials();$now=time();$header=$this->b64(json_encode(['alg'=>'RS256','typ'=>'JWT'],JSON_THROW_ON_ERROR));$claims=$this->b64(json_encode(['iss'=>$c['client_email'],'scope'=>'https://www.googleapis.com/auth/firebase.messaging','aud'=>'https://oauth2.googleapis.com/token','iat'=>$now,'exp'=>$now+3600],JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR));$unsigned=$header.'.'.$claims;$signature='';
        if(!openssl_sign($unsigned,$signature,$c['private_key'],OPENSSL_ALGO_SHA256))throw new RuntimeException('Não foi possível autenticar o FCM.');
        $jwt=$unsigned.'.'.$this->b64($signature);$payload=http_build_query(['grant_type'=>'urn:ietf:params:oauth:grant-type:jwt-bearer','assertion'=>$jwt]);$response=$this->request('https://oauth2.googleapis.com/token',['Content-Type: application/x-www-form-urlencoded'],$payload);$data=json_decode($response['body'],true);
        if($response['http']<200||$response['http']>=300||empty($data['access_token']))throw new RuntimeException('Não foi possível autenticar no Firebase Cloud Messaging.');
        $ttl=max(300,(int)($data['expires_in']??3600));self::$cachedAccessToken=['token'=>(string)$data['access_token'],'expires_at'=>$now+$ttl];return(string)$data['access_token'];
    }
