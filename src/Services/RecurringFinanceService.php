...
final class RecurringFinanceService
{
    ...
    public function save(?int$id,string$direction,string$description,int$amountCents,string$frequency,int$intervalCount,string$nextDueDate,?string$endDate,?int$unitId,?int$eventId,?int$accountId,?int$categoryId,bool$active):int
    {
        ...
        if($accountId){$q=$pdo->prepare('SELECT id FROM financial_accounts WHERE id=? AND tenant_id=? AND active=1');$q->execute([$accountId,$tenantId]);if(!$q->fetchColumn())throw new RuntimeException('Conta inválida ou inativa.');}
        if($categoryId){$q=$pdo->prepare('SELECT direction FROM financial_categories WHERE id=? AND tenant_id=? AND active=1');$q->execute([$categoryId,$tenantId]);$catDir=$q->fetchColumn();if($catDir===false)throw new RuntimeException('Categoria inválida.');if((string)$catDir!==$direction)throw new RuntimeException('A categoria não corresponde ao tipo do lançamento.');}
    ...
    {
        Auth::requirePermission('finance.manage');$tenantId=Auth::tenantId();if(!$tenantId)throw new RuntimeException('Empresa inválida.');$pdo=Database::connection();$q=$pdo->prepare('SELECT active FROM financial_recurring_templates WHERE id=? AND tenant_id=?');$q->execute([$id,$tenantId]);$current=$q->fetchColumn();if($current===false)throw new RuntimeException('Recorrência não encontrada.');if((bool)$current===$active)return;$pdo->prepare('UPDATE financial_recurring_templates SET active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?')->execute([$active?1:0,$id,$tenantId]);Auth::audit('finance.recurring_status','financial_recurring_template',(string)$id,['active'=>$active]);
    }
    ...
    public function generateThrough(int$tenantId,string$throughDate):int
    {
        ...
        while($guard++<500){
            $q=$pdo->prepare('SELECT id FROM financial_recurring_templates WHERE tenant_id=? AND active=1 AND next_due_date<=? AND (end_date IS NULL OR next_due_date<=end_date) ORDER BY next_due_date,id LIMIT 50');$q->execute([$tenantId,$through]);$ids=array_map('intval',array_column($q->fetchAll(),'id'));if(!$ids)break;$progress=false;
            foreach($ids as$id){
                try{$did=Database::transaction(fn(PDO$tx):bool=>$this->generateOne($tx,$tenantId,$id,$through));if($did){$generated++;$progress=true;}}catch(\Throwable){}
            }
            if(!$progress)break;
        }
        return$generated;
