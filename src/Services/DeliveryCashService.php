...
final class DeliveryCashService
{
    ...
    public function resolveHandoff(string $token):array
    {
        ...
        $s=$pdo->prepare('SELECT h.*,u.name delivery_name,ws.unit_id,ou.name unit_name FROM delivery_cash_handoffs h JOIN users u ON u.id=h.delivery_user_id AND u.tenant_id=h.tenant_id JOIN work_shifts ws ON ws.id=h.work_shift_id AND ws.tenant_id=h.tenant_id LEFT JOIN operating_units ou ON ou.id=ws.unit_id AND ou.tenant_id=h.tenant_id WHERE h.tenant_id=? AND h.token_hash=? LIMIT 1');$s->execute([$tenantId,$hash]);$row=$s->fetch();if(!$row)throw new RuntimeException('Repasse não encontrado.');
    ...
    public function confirmHandoff(string $token):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$cashierId,$hash):array{
            $s=$pdo->prepare(Database::portableSql($pdo,'SELECT h.*,u.name delivery_name,ws.unit_id,ou.name unit_name FROM delivery_cash_handoffs h JOIN users u ON u.id=h.delivery_user_id AND u.tenant_id=h.tenant_id JOIN work_shifts ws ON ws.id=h.work_shift_id AND ws.tenant_id=h.tenant_id LEFT JOIN operating_units ou ON ou.id=ws.unit_id AND ou.tenant_id=h.tenant_id WHERE h.tenant_id=? AND h.token_hash=? FOR UPDATE'));$s->execute([$tenantId,$hash]);$h=$s->fetch();if(!$h)throw new RuntimeException('Repasse não encontrado.');if($h['status']==='confirmed')return $h;if($h['status']!=='pending')throw new RuntimeException('Repasse não está pendente.');
            ...
            $pdo->prepare('UPDATE delivery_cash_handoffs SET status="confirmed",confirmed_by=?,confirmed_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=? AND status="pending"')->execute([$cashierId,$h['id']]);
