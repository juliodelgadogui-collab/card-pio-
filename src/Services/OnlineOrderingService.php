...
final class OnlineOrderingService
{
    ...
    public function statusAt(PDO $pdo,int $tenantId,string $channel,?\DateTimeImmutable $when=null):array
    {
        ...
        if($channel==='pickup'&&array_key_exists('pickup_enabled',$settings)&&empty($settings['pickup_enabled']))return ['open'=>false,'reason'=>'Retirada no local indisponível.'];
    ...
    {
        $sql='SELECT status,settings FROM tenants WHERE id=? LIMIT 1'.($forUpdate?' FOR UPDATE':'');
    ...
    private function matchesRule(\DateTimeImmutable $when,mixed $rule,int $dayOffset,bool $onlyOvernight=false):bool
    {
        ...
        $open=$this->validTime((string)($rule['open']??''));$close=$this->validTime((string)($rule['close']??''));
        if($open===null||$close===null)return false;$overnight=$close<=$open;if($onlyOvernight&&!$overnight)return false;
        ...
        $from=$base->setTime($oh,$om);$to=$base->setTime($ch,$cm);if($overnight)$to=$to->modify('+1 day');
        return $when>=$from&&$when<$to;
    }

    private function validTime(string $value):?string
    {
        $value=trim($value);return preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',$value)?$value:null;
    }
}
