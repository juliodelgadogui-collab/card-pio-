...
final class OrderCreationService
{
    public function create(array $payload):array
    {
        ...
        $order=Database::transaction(function(PDO $tx)use($tenantId,$userId,$channel,$tableId,$eventId,$name,$phone,$address,$notes,$quantities):array{
            ...
            $token=bin2hex(random_bytes(20));$o=$tx->prepare('INSERT INTO orders (public_token,tenant_id,customer_id,channel,status,payment_status,subtotal_cents,total_cents,table_id,tab_id,event_id,delivery_address,notes,created_by) VALUES (?,?,?,?,"confirmed","unpaid",?,?,?,?,?,?,?,?)');$o->execute([$token,$tenantId,$customerId,$channel,$total,$total,$validTableId,$tabId,$validEventId,$channel==='delivery'?$address:null,$notes?:null,$userId]);$orderId=(int)$tx->lastInsertId();
            ...
            return ['id'=>$orderId,'public_token'=>$token,'channel'=>$channel,'status'=>'confirmed','payment_status'=>'unpaid','total_cents'=>$total,'customer_id'=>$customerId,'table_id'=>$validTableId,'tab_id'=>$tabId,'event_id'=>$validEventId];
        });
