    ...
    public function enrich(array $orders):array
    {
        ...
        try{
            ...
            $sql='SELECT o.id,o.assigned_delivery_user_id,u.name delivery_name,dp.picked_up_at,dp.route_started_at,dp.arrived_at,dp.completed_at,ll.updated_at location_received_at
                ...
                LEFT JOIN delivery_live_locations ll ON ll.order_id=o.id AND ll.tenant_id=o.tenant_id
                WHERE o.tenant_id=? AND o.unit_id=? AND o.id IN ('.$placeholders.')';
            $s=Database::connection()->prepare($sql);
            $s->execute(array_merge([$tenantId,$unitId],$ids));
            $delivery=[];
            foreach($s->fetchAll()as$row)$delivery[(int)$row['id']]=$row;
        }catch(\Throwable){
            return $orders;
        }
        ...
        foreach($orders as&$order){
            ...
            $d=$delivery[(int)$order['id']]??[];
            $assigned=!empty($d['assigned_delivery_user_id']);
            $picked=!empty($d['picked_up_at']);
            $route=!empty($d['route_started_at']);
            $arrived=!empty($d['arrived_at']);
            $completed=!empty($d['completed_at']) || (string)($order['status']??'')==='completed';

            $stage=$completed?'Entregue':(
                !$assigned?'Sem entregador':(
                    !$picked?'Aguardando retirada':(
                        !$route?'Retirado':(
                            !$arrived?'Em rota':'Chegou'
                        )
                    )
                )
            );
            ...
            $order['delivery_completed']=$completed;
