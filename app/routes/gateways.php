...
?><p class="muted">Segredos são criptografados com APP_KEY e nunca são exibidos novamente pelo painel.</p><div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(300px,1fr))"><?php foreach(['stripe'=>'Stripe','pagbank'=>'PagBank','mercadopago'=>'Mercado Pago'] as $provider=>$label):$row=$gateways[$provider]??null;?><section class="card"><div class="section-head"><h2><?= $label ?></h2><span class="badge"><?= $row&&$row['active']?'Ativo':'Inativo' ?></span></div><form method="post" class="form-grid"><input type="hidden" name="_csrf" value="<?= em_csrf() ?>"><input type="hidden" name="action" value="gateway-save"><input type="hidden" name="provider" value="<?= $provider ?>"><?php if($provider==='stripe'):?><label class="span-2">Account ID (acct_...)<input name="account_reference" value="<?= Security::e($row['account_reference']??'') ?>" required></label><label class="span-2">Secret key<input type="password" name="secret_key" placeholder="Deixe vazio para manter a atual"></label><label class="span-2">Segredo do webhook<input type="password" name="webhook_secret" placeholder="Deixe vazio para manter o atual"></label><?php elseif($provider==='mercadopago'):?><label class="span-2">Collector ID<input name="account_reference" value="<?= Security::e($row['account_reference']??'') ?>" required></label><label class="span-2">Access token<input type="password" name="access_token" placeholder="Deixe vazio para manter o atual"></label><label class="span-2">Segredo do webhook<input type="password" name="webhook_secret" placeholder="Deixe vazio para manter o atual"></label><?php else:?><label class="span-2">Vínculo da credencial<input value="<?= Security::e($row['account_reference']??'Será gerado ao salvar o token') ?>" disabled></label><label class="span-2">Token PagBank<input type="password" name="token" placeholder="Deixe vazio para manter o atual"></label><label class="span-2">API base<input name="api_base" placeholder="https://api.pagseguro.com (produção) ou sandbox"></label><p class="muted span-2">O PagBank valida webhook com SHA-256 do próprio token + payload. Por isso não existe um segundo segredo de webhook neste painel; o EventMenu também vincula a configuração por fingerprint do token, sem armazenar ou exibir o token em texto puro.</p><?php endif;?><label class="checkbox span-2"><input type="checkbox" name="active"<?= em_checked($row['active']??0) ?>> Gateway ativo</label><button class="primary span-2">Salvar <?= $label ?></button></form><p class="muted">Webhook: <code><?= Security::e(rtrim((string)env('APP_URL',''),'/').'/webhook.php?provider='.$provider.'&tenant='.$tenantSlug) ?></code></p></section><?php endforeach;?></div>
