...
function orders_is_eventmenu_delivery(array$order):bool{return strtoupper(trim((string)($order['order_source']??'')))==='EVENTMENU_DELIVERY';}
...
<section class="card" style="margin-bottom:14px">
    <div class="section-head"><div><span class="eyebrow"><?= Security::e(orders_channel((string)$viewOrder['channel'])) ?> · <?= Security::e($unit['name']) ?></span><h2>Pedido #<?= $viewId ?></h2><?php if(orders_is_eventmenu_delivery($viewOrder)):?><span class="badge">EventMenu Delivery</span><?php endif;?><div class="muted"><?= Security::e($viewOrder['customer_name']??$viewOrder['table_name']??'Consumidor') ?> · <?= Security::e(orders_status_label((string)$viewOrder['status'])) ?> · <?= Security::e(orders_payment_label((string)$viewOrder['payment_status'])) ?></div></div><a class="button secondary" href="<?= Security::e(app_url('?route=orders')) ?>">Fechar detalhes</a></div>
