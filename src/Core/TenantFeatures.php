...
final class TenantFeatures
{
    ...
    public static function routeEnabled(string$route,?int$tenantId=null):bool{$menu=['pos','cash','kitchen','kds-stream','production','product-config','delivery','pickup','products','inventory','purchases','restaurant'];$event=['events','event-admin','tickets','guests','promoters'];if(in_array($route,$menu,true))return self::menu($tenantId);if(in_array($route,$event,true))return self::events($tenantId);return true;}
}
