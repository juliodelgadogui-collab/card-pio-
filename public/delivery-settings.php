...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        $webhookSecret=trim((string)($_POST['mp_webhook_secret']??''));$existingWebhookEncrypted=(string)($gateway['webhook_secret_encrypted']??'');$webhookEncrypted=$existingWebhookEncrypted;
        if($webhookSecret!=='')$webhookEncrypted=Crypto::encrypt($webhookSecret);
        $pixEnabled=!empty($_POST['mp_pix_enabled']);$cardEnabled=!empty($_POST['mp_card_enabled']);
        $creditEnabled=$cardEnabled&&!empty($_POST['mp_credit_enabled']);$debitEnabled=$cardEnabled&&!empty($_POST['mp_debit_enabled']);$activeRequested=!empty($_POST['mp_active']);
        if($cardEnabled&&!$creditEnabled&&!$debitEnabled)throw new RuntimeException('Escolha Crédito, Débito ou os dois para aceitar cartão no aplicativo.');
        $account=trim((string)($_POST['mp_account_reference']??($gateway['account_reference']??'')));
        if($access!==''&&$account==='')$account=$discoverMercadoPagoAccount($access);
        if($activeRequested&&$access==='')throw new RuntimeException('Informe o Access Token do Mercado Pago antes de ativar a conta.');
        if($activeRequested&&($pixEnabled||$cardEnabled)&&$account==='')throw new RuntimeException('Não foi possível identificar a conta recebedora do Mercado Pago. Confira o Access Token ou informe o Collector ID.');
        if($activeRequested&&($pixEnabled||$cardEnabled)&&$webhookEncrypted==='')throw new RuntimeException('Informe a assinatura secreta do webhook do Mercado Pago. Ela é necessária para confirmar pagamentos com segurança.');
        if($cardEnabled&&$public==='')throw new RuntimeException('Informe a Public Key para aceitar cartão dentro do aplicativo.');

        $mp['access_token']=$access;$mp['public_key']=$public;$mp['pix_enabled']=$pixEnabled;$mp['card_enabled']=$cardEnabled;$mp['credit_enabled']=$creditEnabled;$mp['debit_enabled']=$debitEnabled;$mp['max_installments']=max(1,min(12,(int)($_POST['mp_max_installments']??12)));
        $encrypted=Crypto::encryptJson($mp);$active=($activeRequested&&$access!==''&&$account!==''&&$webhookEncrypted!=='')?1:0;
...
$creditChecked=!array_key_exists('credit_enabled',$mp)||!empty($mp['credit_enabled']);$debitChecked=!array_key_exists('debit_enabled',$mp)||!empty($mp['debit_enabled']);
...
<section class="card"><div class="section-head"><div><span class="eyebrow">PAGAMENTO NO APP</span><h3>Mercado Pago</h3></div><span class="status-pill <?=!empty($gateway['active'])?'active':''?>"><?=!empty($gateway['active'])?'Ativo':'Configurar'?></span></div><p class="muted">O Access Token e o segredo do webhook ficam criptografados no servidor. O aplicativo recebe somente a Public Key necessária para tokenizar o cartão; número e CVV não passam pelo EventMenu. Aproximação/NFC não usa Mercado Pago neste fluxo.</p>
<label><input type="checkbox" name="mp_active" value="1" <?=!empty($gateway['active'])?'checked':''?>> Conta Mercado Pago ativa</label>
<div class="form-grid"><label>Public Key<input name="mp_public_key" value="<?=Security::e((string)($mp['public_key']??''))?>" autocomplete="off" placeholder="APP_USR-..."></label><label>Access Token<input name="mp_access_token" type="password" value="" autocomplete="new-password" placeholder="<?=!empty($mp['access_token'])?'Token já salvo — deixe vazio para manter':'Informe o Access Token'?>"></label><label class="span-2">Collector ID / conta recebedora<input name="mp_account_reference" value="<?=Security::e((string)($gateway['account_reference']??''))?>" inputmode="numeric" placeholder="Detectado automaticamente ao salvar"><small>O EventMenu tenta detectar pelo Access Token. Este identificador é usado para impedir que um pagamento de outra conta seja aceito.</small></label><label class="span-2">Assinatura secreta do webhook<input name="mp_webhook_secret" type="password" value="" autocomplete="new-password" placeholder="<?=$hasWebhookSecret?'Segredo já salvo — deixe vazio para manter':'Cole a assinatura secreta do webhook'?>"><small>Cadastre no Mercado Pago o webhook do EventMenu e copie a assinatura secreta. Sem ela, PIX/cartão pendente não pode ser confirmado pelo servidor.</small></label><label><input type="checkbox" name="mp_pix_enabled" value="1" <?=!array_key_exists('pix_enabled',$mp)||!empty($mp['pix_enabled'])?'checked':''?>> PIX dentro do app</label><label><input type="checkbox" name="mp_card_enabled" value="1" <?=!empty($mp['card_enabled'])?'checked':''?>> Cartão dentro do app</label><div class="span-2 card" style="padding:14px"><strong>Tipos de cartão</strong><p class="muted">O SDK identifica o tipo real pelo cartão. Débito é sempre à vista; crédito usa as parcelas permitidas pela conta.</p><label class="checkbox"><input type="checkbox" name="mp_credit_enabled" value="1" <?=$creditChecked?'checked':''?>> Aceitar crédito</label><label class="checkbox"><input type="checkbox" name="mp_debit_enabled" value="1" <?=$debitChecked?'checked':''?>> Aceitar débito</label></div><label>Máximo de parcelas no crédito<input name="mp_max_installments" type="number" min="1" max="12" value="<?=Security::e((string)($mp['max_installments']??12))?>"></label></div>
<div class="alert"><strong>Webhook Mercado Pago:</strong> <?=Security::e(app_absolute_url('webhook.php?provider=mercadopago&tenant='.(string)$tenant['slug']))?></div></section>
