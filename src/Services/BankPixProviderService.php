...
final class BankPixProviderService
{
    ...
    private function reconcilePayment(PDO $pdo,array $payment,array $config,string $accountReference):array
    {
        ...
        if(!empty($remote['paid'])){
            (new PaymentService())->confirmVerified([
                ...
                'provider_payment_id'=>(string)($remote['provider_payment_id']?:$txid),'amount_cents'=>(int)$payment['amount_cents'],'currency'=>'BRL','account_reference'=>$accountReference,'source'=>'pix','payment_method_type'=>'pix','raw_status'=>(string)$remote['raw_status'],
            ]);
            return ['checked'=>true,'status'=>'paid','terminal'=>true];
        }
        if(!empty($remote['terminal'])){
            Database::transaction(function(PDO $tx)use($payment,$remote):void{
                ...
                $tx->prepare('UPDATE payments SET status=?,raw_payload=? WHERE id=? AND status IN ("created","pending","authorized")')->execute([$status,json_encode(['provider_status'=>$remote['raw_status'],'checked_at'=>gmdate('c')],JSON_UNESCAPED_UNICODE),(int)$payment['id']]);
    ...
    private function createEfi(array $ctx,array $payment,string $taxId,string $key):array
    {
        ...
        $token=$this->efiToken($config,$base);$txid=substr(hash('sha256',$key),0,32);$doc=$this->document($taxId);$debtor=['nome'=>mb_substr((string)$ctx['account']['name'],0,200)];$debtor[strlen($doc)===11?'cpf':'cnpj']=$doc;
    ...
    private function createInter(array $ctx,array $payment,string $taxId,string $key):array
    {
        ...
        $data=$this->mtlsJson('POST',$base.'/pix/v2/cob',$headers,$body,$config);$copy=trim((string)($data['pixCopiaECola']??''));$txid=trim((string)($data['txid']??''));if($copy===''||$txid==='')throw new RuntimeException('Banco Inter não retornou os dados completos da cobrança Pix.');
    ...
    {
        $status=mb_strtoupper(trim((string)($data['status']??'')));$paid=$status==='CONCLUIDA';$terminal=in_array($status,['REMOVIDA_PELO_USUARIO_RECEBEDOR','REMOVIDA_PELO_PSP','REMOVIDA_PELO_USUARIO_PAGADOR'],true);$providerId=$txid;if($paid&&is_array($data['pix']??null)&&!empty($data['pix'][0]['endToEndId']))$providerId=(string)$data['pix'][0]['endToEndId'];return ['paid'=>$paid,'terminal'=>$terminal,'provider_payment_id'=>$providerId,'raw_status'=>$terminal?'cancelled':strtolower($status?:'pending')];
    }
