...
final class Auth
{
    ...
    public static function attempt(string $email,string $password):bool
    {
        ...
        if(!$credentialsValid||$blocked){if(!$blocked)self::registerLoginFailure($pdo,$email);return false;}
        self::clearLoginThrottle($pdo,$email);session_regenerate_id(true);unset($_SESSION['super_admin_tenant_id']);self::setSession($user);$pdo->prepare('UPDATE users SET last_login_at=NOW() WHERE id=?')->execute([$user['id']]);self::audit('auth.login','user',(string)$user['id']);return true;
    }
