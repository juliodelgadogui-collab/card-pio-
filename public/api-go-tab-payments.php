...
try{
    $auth=new ApiAuthService();$token=ApiAuthService::bearerToken();$deviceId=ApiAuthService::deviceId();if($token==='')gotab_out(['ok'=>false,'error'=>'Token Bearer obrigatório.'],401);$auth->authenticate($token,$deviceId);
    ...
    $action=(string)($_GET['action']??'account');$service=new TabSplitPaymentService();

    if($action==='account'){$tabId=(int)($_GET['tab_id']??0);gotab_out(['ok'=>true,'account'=>$service->account($tabId)]);}
    if($action==='group-create'){
        gotab_post();$body=gotab_body();$options=is_array($body['options']??null)?$body['options']:[];$group=$service->create((int)($body['tab_id']??0),(string)($body['split_type']??''),(string)($body['method']??''),$options,(string)($body['idempotency_key']??''));gotab_out(['ok'=>true,'group'=>$group],201);
    }
    if($action==='group-status'){$groupId=(int)($_GET['group_id']??0);gotab_out(['ok'=>true,'group'=>$service->status($groupId)]);}
    if($action==='group-cancel'){gotab_post();$body=gotab_body();gotab_out(['ok'=>true,'group'=>$service->cancel((int)($body['group_id']??0))]);}
    if($action==='pix-create'){gotab_post();$body=gotab_body();gotab_out(['ok'=>true,'pix'=>(new TabSplitPixService())->create((int)($body['group_id']??0),(string)($body['tax_id']??''))],201);}
    if($action==='pix-status'){$groupId=(int)($_GET['group_id']??0);gotab_out(['ok'=>true]+(new TabSplitPixService())->status($groupId));}
    if($action==='nfc-intent'){gotab_post();$body=gotab_body();gotab_out(['ok'=>true]+(new TabSplitNfcService())->createIntent((int)($body['group_id']??0),$deviceId),201);}
    if($action==='nfc-verify'){gotab_post();$body=gotab_body();gotab_out((new TabSplitNfcService())->verifyIntent((string)($body['intent_token']??''),(string)($body['transaction_code']??''),$deviceId));}
    gotab_out(['ok'=>false,'error'=>'Endpoint de conta dividida não encontrado.'],404);
}catch(RuntimeException $e){gotab_out(['ok'=>false,'error'=>$e->getMessage()],422);}catch(Throwable $e){if(filter_var(env('APP_DEBUG','false'),FILTER_VALIDATE_BOOL))gotab_out(['ok'=>false,'error'=>$e->getMessage()],500);gotab_out(['ok'=>false,'error'=>'Erro interno.'],500);}
