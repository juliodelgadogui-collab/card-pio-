...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='points-adjust'){
        if(!Auth::can('loyalty.adjust'))exit('Seu perfil não pode ajustar saldo de pontos.');
        $id=(int)($_POST['id']??0);$points=(int)($_POST['points']??0);$reason=mb_substr(trim((string)($_POST['reason']??'')),0,300);
        if($points===0)exit('Informe pontos diferentes de zero.');if(mb_strlen($reason)<5)exit('Informe o motivo do ajuste com pelo menos 5 caracteres.');
        $key='adjust:'.Auth::id().':'.$id.':'.bin2hex(random_bytes(8));
        ...
        });Auth::audit('customer.points_adjusted','customer',(string)$id,['points'=>$points,'reason'=>$reason]);em_flash('ok','Pontos ajustados e motivo registrado na auditoria.');}catch(Throwable$e){em_flash('error',$e->getMessage());}em_go('customers',['view'=>$id]);
    }

    if($action==='points-apply'){
        if(!Auth::can('loyalty.redeem'))exit('Seu perfil não pode usar pontos em pedidos.');
    ...
    if($action==='points-remove'){
        if(!Auth::can('loyalty.redeem'))exit('Seu perfil não pode alterar o resgate de pontos do pedido.');
...
<section class="card" style="margin-bottom:16px">
  ...
  <?php if($pointsConfig['enabled']&&Auth::can('loyalty.redeem')):?>
  ...
  <?php elseif($pointsConfig['enabled']):?><div class="alert">Seu perfil pode consultar o saldo, mas não pode usar ou remover pontos de pedidos.</div><?php endif;?>
