...
final class TicketService
{
    public function reservePublic(int $eventId, int $batchId, int $quantity, array $buyer, ?string $couponCode = null, ?string $promoterCode = null): array
    {
        ...
        return Database::transaction(function (PDO $pdo) use ($eventId, $batchId, $quantity, $buyer, $couponCode, $promoterCode): array {
            ...
            if (!empty($event['capacity_total'])) {
                $cap=$pdo->prepare('SELECT COUNT(*) FROM tickets WHERE tenant_id=? AND event_id=? AND status IN ("reserved","paid","checked_in")');$cap->execute([$event['tenant_id'],$eventId]);$used=(int)$cap->fetchColumn();if($used+$quantity>(int)$event['capacity_total'])throw new RuntimeException('Capacidade total do evento atingida.');
            }
            if (!empty($batch['ticket_type_id']) && !empty($batch['ticket_type_capacity'])) {
                $cap=$pdo->prepare('SELECT COUNT(*) FROM tickets t JOIN ticket_batches b ON b.id=t.batch_id WHERE t.tenant_id=? AND t.event_id=? AND b.ticket_type_id=? AND t.status IN ("reserved","paid","checked_in")');$cap->execute([$event['tenant_id'],$eventId,$batch['ticket_type_id']]);$used=(int)$cap->fetchColumn();if($used+$quantity>(int)$batch['ticket_type_capacity'])throw new RuntimeException('Capacidade do tipo de ingresso atingida.');
            }
            ...
            if ($couponCode) {
                $code = strtoupper(trim($couponCode));$restricted=$pdo->prepare('SELECT COUNT(*) FROM event_coupons WHERE tenant_id=? AND event_id=? AND active=1');$restricted->execute([$event['tenant_id'],$eventId]);$hasRestrictions=(int)$restricted->fetchColumn()>0;
                $sql='SELECT c.* FROM coupons c WHERE c.tenant_id=? AND c.code=? AND c.active=1';if($hasRestrictions)$sql.=' AND EXISTS (SELECT 1 FROM event_coupons ec WHERE ec.tenant_id=c.tenant_id AND ec.event_id=? AND ec.coupon_id=c.id AND ec.active=1)';$sql=Database::portableSql($pdo,$sql.' FOR UPDATE');$c=$pdo->prepare($sql);$args=[$event['tenant_id'],$code];if($hasRestrictions)$args[]=$eventId;$c->execute($args);$coupon=$c->fetch();
            ...
            $promoterId = null;
            if ($promoterCode) {
                $restricted=$pdo->prepare('SELECT COUNT(*) FROM event_promoters WHERE tenant_id=? AND event_id=? AND active=1');$restricted->execute([$event['tenant_id'],$eventId]);$hasRestrictions=(int)$restricted->fetchColumn()>0;
                $sql='SELECT p.id FROM promoters p WHERE p.tenant_id=? AND p.code=? AND p.active=1';if($hasRestrictions)$sql.=' AND EXISTS (SELECT 1 FROM event_promoters ep WHERE ep.tenant_id=p.tenant_id AND ep.event_id=? AND ep.promoter_id=p.id AND ep.active=1)';$p=$pdo->prepare($sql);$args=[$event['tenant_id'],strtoupper(trim($promoterCode))];if($hasRestrictions)$args[]=$eventId;$p->execute($args);$promoterId=$p->fetchColumn()?:null;if(!$promoterId)throw new RuntimeException('Código de promotor inválido para este evento.');
            }

            $total = max(0, $subtotal - $discount);$publicToken = bin2hex(random_bytes(20));
            ...
            $stmt->execute([$publicToken, $event['tenant_id'], $customerId, $couponId, $promoterId, $subtotal, $discount, $total, 'Evento #' . $eventId]);$orderId = (int)$pdo->lastInsertId();
            $typeLabel=trim((string)($batch['ticket_type_name']??''));$itemName='Ingresso '.$event['name'].' — '.($typeLabel!==''?$typeLabel.' · ':'').$batch['name'];$pdo->prepare('INSERT INTO order_items (order_id,product_id,name_snapshot,unit_price_cents,quantity,total_cents) VALUES (?,NULL,?,?,?,?)')->execute([$orderId,$itemName,$batch['price_cents'],$quantity,$subtotal]);
            ...
            $paid = false;
            if ($total === 0) {
                $pdo->prepare('UPDATE orders SET status="confirmed",payment_status="paid" WHERE id=? AND tenant_id=?')->execute([$orderId, $event['tenant_id']]);$pdo->prepare('UPDATE tickets SET status="paid",reserved_until=NULL WHERE tenant_id=? AND order_id=? AND status="reserved"')->execute([$event['tenant_id'], $orderId]);$pdo->prepare(Database::portableSql($pdo, 'UPDATE ticket_batches SET quantity_reserved=GREATEST(0,quantity_reserved-?),quantity_sold=quantity_sold+? WHERE id=?'))->execute([$quantity, $quantity, $batchId]);
    ...
    public function checkIn(string $token): array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$token):array{
            $s=$pdo->prepare(Database::portableSql($pdo,'SELECT t.*,e.status event_status,tn.status tenant_status FROM tickets t JOIN events e ON e.id=t.event_id JOIN tenants tn ON tn.id=t.tenant_id WHERE t.tenant_id=? AND (t.qr_token=? OR t.code=?) LIMIT 1 FOR UPDATE'));$s->execute([$tenantId,$token,$token]);$ticket=$s->fetch();if(!$ticket)throw new RuntimeException('Ingresso não encontrado.');
            ...
            if($ticket['event_status']!=='published'){$this->logCheckin($pdo,$tenantId,(int)$ticket['id'],'blocked',['reason'=>'event_'.$ticket['event_status']]);return ['ok'=>false,'result'=>'blocked','ticket'=>$ticket];}
            ...
            $pdo->prepare('UPDATE tickets SET status="checked_in",checked_in_at=CURRENT_TIMESTAMP,checked_in_by=? WHERE id=?')->execute([Auth::id(),$ticket['id']]);$this->logCheckin($pdo,$tenantId,(int)$ticket['id'],'accepted');$this->eventAudit($pdo,$tenantId,(int)$ticket['event_id'],Auth::id(),'ticket.checkin','ticket',(string)$ticket['id'],[]);$ticket['status']='checked_in';return ['ok'=>true,'result'=>'accepted','ticket'=>$ticket];
        });
    }
    ...
    public function releaseExpired(): int
    {
        return Database::transaction(function(PDO $pdo):int{
            $s=$pdo->query(Database::portableSql($pdo,'SELECT batch_id,COUNT(*) qty FROM tickets WHERE status="reserved" AND reserved_until IS NOT NULL AND reserved_until<CURRENT_TIMESTAMP GROUP BY batch_id FOR UPDATE'));$groups=$s->fetchAll();$total=0;foreach($groups as$g){$pdo->prepare(Database::portableSql($pdo,'UPDATE ticket_batches SET quantity_reserved=GREATEST(0,quantity_reserved-?) WHERE id=?'))->execute([(int)$g['qty'],$g['batch_id']]);$total+=(int)$g['qty'];}
            $couponRows=$pdo->query(Database::portableSql($pdo,'SELECT id,coupon_id FROM coupon_reservations WHERE status="reserved" AND expires_at<CURRENT_TIMESTAMP FOR UPDATE'))->fetchAll();foreach($couponRows as$row){$pdo->prepare('UPDATE coupon_reservations SET status="released" WHERE id=?')->execute([$row['id']]);$pdo->prepare(Database::portableSql($pdo,'UPDATE coupons SET reserved_count=GREATEST(0,reserved_count-1) WHERE id=?'))->execute([$row['coupon_id']]);}
            $pdo->exec('UPDATE tickets SET status="cancelled" WHERE status="reserved" AND reserved_until IS NOT NULL AND reserved_until<CURRENT_TIMESTAMP');$pdo->exec('UPDATE orders SET status="cancelled",payment_status=CASE WHEN payment_status="unpaid" THEN "failed" ELSE payment_status END WHERE channel="event" AND payment_status<>"paid" AND NOT EXISTS (SELECT 1 FROM tickets t WHERE t.order_id=orders.id AND t.status="reserved") AND EXISTS (SELECT 1 FROM tickets t2 WHERE t2.order_id=orders.id)');return $total;
        });
    }
