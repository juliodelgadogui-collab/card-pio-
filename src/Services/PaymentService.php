...
{
    private const PROVIDERS=['stripe','pagbank','mercadopago','pagarme','asaas','openpix','efi','sumup','cielo','manual','terminal_manual'];
    ...
    public function create(int $orderId,string $provider,string $idempotencyKey,?int $amountCents=null):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$orderId,$provider,$idempotencyKey,$amountCents):array{
            ...
            if(!in_array($provider,['manual','terminal_manual'],true)){$gw=$pdo->prepare('SELECT id FROM payment_gateways WHERE tenant_id=? AND provider=? AND active=1');$gw->execute([$tenantId,$provider]);if(!$gw->fetchColumn())throw new RuntimeException('Gateway não está ativo para esta empresa.');}
    ...
    public function confirmVerified(array $verified):void
    {
        ...
        Database::transaction(function(PDO $pdo)use($verified):void{
            ...
            if(!in_array($provider,['manual','terminal_manual'],true)){
    ...
    private function publishVerifiedNotification(array $verified):void
    {
        ...
        try{
            ...
            $title=str_contains($source,'pix')?'PIX recebido':(str_contains($source,'nfc')||str_contains($source,'card')||str_contains($source,'tap')||$provider==='terminal_manual'?'Cartão aprovado':'Pagamento confirmado');
