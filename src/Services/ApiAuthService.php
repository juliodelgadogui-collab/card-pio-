...
final class ApiAuthService
{
    public function login(string $email,string $password,string $deviceId,string $deviceLabel=''):array
    {
        ...
        $stmt=$pdo->prepare('SELECT u.*,t.status tenant_status,t.name tenant_name FROM users u JOIN tenants t ON t.id=u.tenant_id WHERE u.email=? LIMIT 1');$stmt->execute([$email]);$user=$stmt->fetch();
        ...
        return ['token'=>$raw,'expires_at'=>$expires,'user'=>['id'=>(int)$user['id'],'tenant_id'=>(int)$user['tenant_id'],'tenant_name'=>(string)$user['tenant_name'],'name'=>(string)$user['name'],'email'=>(string)$user['email'],'role'=>(string)$user['role']]];
    }
