...
final class Auth
{
    ...
    private static function appBasePath():string
    {
        ...
        // Em hospedagem compartilhada, a URL real requisitada tem prioridade.
        // Isso evita sair de /1 para a raiz do domínio quando APP_URL ficou sem subpasta.
    ...
    public static function enforceCurrentUser():void
    {
        ...
        if(!$user||$user['status']!=='active'||($user['tenant_id']!==null&&$user['tenant_status']!=='active')){
            self::logout();
            header('Location: '.self::appUrl('/?route=login&blocked=1'));
            exit;
        }
    ...
    {
        if(!self::check()){
            header('Location: '.self::appUrl('/?route=login'));
            exit;
        }
