...
final class PosPaymentService
{
    ...
    public function externalTerminal(int $orderId,int $amountCents,string $paymentMethod,string $machineLabel,string $transactionReference,string $idempotencyKey,string $deviceId=''):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$userId,$orderId,$amountCents,$paymentMethod,$machineLabel,$transactionReference,$idempotencyKey,$deviceId):array{
            ...
            $pdo->prepare('INSERT INTO external_terminal_payments (tenant_id,unit_id,order_id,payment_id,user_id,device_id,payment_method,machine_label,transaction_reference,amount_cents) VALUES (?,?,?,?,?,?,?,?,?,?)')
                ->execute([$tenantId,$unitId!==null?(int)$unitId:null,$orderId,(int)$payment['id'],$userId,$deviceId!==''?$deviceId:null,$paymentMethod,$machineLabel!==''?$machineLabel:null,$transactionReference,(int)$payment['amount_cents']]);
            Auth::audit('payment.external_terminal_confirmed','payment',(string)$payment['id'],['order_id'=>$orderId,'amount_cents'=>(int)$payment['amount_cents'],'payment_method'=>$paymentMethod,'machine_label'=>$machineLabel,'transaction_reference'=>$transactionReference,'user_id'=>$userId]);
            return $this->status($orderId);
        });
    }
}
