...
function eventmenu_install_write_env(string $path, array $values): void
{
    ...
    if (file_put_contents($path, $contents, LOCK_EX) === false) {
        throw new RuntimeException('Não foi possível criar o arquivo .env. Verifique a permissão de escrita da pasta do sistema.');
    }
    @chmod($path, 0640);
    eventmenu_install_runtime_env($values);
}

// Servidor zerado: antes de existir .env, o instalador trabalha em SQLite por padrão.
if (!is_file($envPath)) {
    eventmenu_install_runtime_env([
        'APP_ENV' => 'production',
        'APP_DEBUG' => 'false',
        'DB_CONNECTION' => 'sqlite',
        'DB_SQLITE_PATH' => 'storage/eventmenu.sqlite',
...
if (!$alreadyInstalled) {
    try {
        ...
        // Banco/tabelas ainda não existem: instalação inicial permitida.
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::validateCsrf($_POST['_csrf'] ?? null)) {
        ...
        $error = 'O servidor ainda não atende todos os requisitos exibidos abaixo.';
    } else {
        try {
            ...
            if (!is_file($envPath)) {
                $initialEnv = eventmenu_install_build_env((string)($_POST['app_url'] ?? $detectedUrl));
                eventmenu_install_write_env($envPath, $initialEnv);
            }

            if (strtolower((string)env('DB_CONNECTION', 'sqlite')) !== 'sqlite') {
                throw new RuntimeException('A primeira instalação deste pacote está preparada para SQLite. Ajuste DB_CONNECTION=sqlite no .env.');
            }
            ...
            if (file_put_contents($lock, date(DATE_ATOM), LOCK_EX) === false) {
                throw new RuntimeException('Instalação criada no banco, mas não foi possível gravar o arquivo de lock. Não execute o instalador novamente.');
            }
            @chmod($lock, 0640);
            $success = true;
        } catch (Throwable $e) {
...
<head>
    ...
    <title>Instalar EventMenu Premium</title>
    <link rel="stylesheet" href="<?= Security::e(app_url('assets/app.css')) ?>">
</head>
<body class="auth-page">
<main class="auth-card">
    <div class="brand">EventMenu <span>Premium</span></div>
    <h1>Instalação inicial</h1>

    <?php if ($success): ?>
        <div class="alert ok">Instalado com sucesso usando SQLite. O banco foi criado em <strong>storage/eventmenu.sqlite</strong> e o usuário informado é o Super ADM da plataforma. <a href="<?= Security::e(app_url('')) ?>">Entrar no sistema</a>.</div>
    <?php else: ?>
        <?php if ($error): ?><div class="alert error"><?= Security::e($error) ?></div><?php endif; ?>

        <h2>Verificação do servidor</h2>
        <ul>
            <?php foreach ($preflight as $label => $ok): ?>
                <li><?= $ok ? '✅' : '❌' ?> <?= Security::e($label) ?></li>
            <?php endforeach; ?>
        </ul>

        <?php if ($preflightOk): ?>
            <p class="muted">O instalador criará automaticamente o arquivo <strong>.env</strong>, chaves privadas aleatórias, a pasta <strong>storage</strong> e o banco SQLite. Não é necessário criar banco manualmente.</p>
            <form method="post">
                ...
                <label>Empresa inicial<input name="tenant_name" value="<?= Security::e((string)($_POST['tenant_name'] ?? '')) ?>" required></label>
                <label>Nome do Super ADM<input name="name" value="<?= Security::e((string)($_POST['name'] ?? '')) ?>" required></label>
                <label>E-mail do Super ADM<input name="email" type="email" value="<?= Security::e((string)($_POST['email'] ?? '')) ?>" required></label>
                <label>Senha do Super ADM<input name="password" type="password" minlength="10" required></label>
                <button class="primary" type="submit">Instalar EventMenu com SQLite</button>
            </form>
        <?php else: ?>
            <div class="alert error">Corrija os itens marcados com ❌ no servidor antes de continuar.</div>
        <?php endif; ?>
    <?php endif; ?>
</main>
</body>
