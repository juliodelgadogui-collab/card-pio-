...
final class MarketplaceConsumerService
{
    public function createOrder(PDO $pdo, string $entryToken, array $payload): array
    {
        ...
        $items = $payload['items'] ?? [];
        if (!is_array($items) || !$items || count($items) > 80) {
        ...
        if ($name === '' || $phone === '' || $address === '') {
            throw new RuntimeException('Informe nome, telefone e endereço para a entrega.');
        }
    ...
    public function publicOrderByToken(PDO $pdo, string $publicToken): array
    {
        ...
        // SQL-standard single-quoted literal keeps this portable on SQLite.
        // The joined operating_units table has a column named `active`, so
        // double-quoted "active" can be interpreted as an identifier there.
        ...
        if ((string)$order['channel'] === 'delivery') {
            try {
                $existing = (new DeliveryPublicTrackingService())->existingForPublicOrder(
                    (int)$order['tenant_id'],
                    (int)$order['id'],
                    $publicToken
                );
