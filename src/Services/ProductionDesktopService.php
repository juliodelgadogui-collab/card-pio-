...
final class ProductionDesktopService
{
    ...
    {
        $this->requireOperation();
        return(new ProductionService())->changeJobStatus($jobId,$status,$reason);
    }
    ...
    public function claimPrint(string$deviceId):?array
    {
        ...
        return Database::transaction(function(PDO$pdo)use($tenantId,$unitId,$deviceId):?array{
            $sql=Database::portableSql($pdo,'SELECT q.id,q.station_id,q.order_id,q.queue_type,q.status,q.attempts,q.payload_hash,ps.name station_name,ps.station_type,ps.printer_target,ps.desktop_device_id,o.channel,o.status order_status,o.notes order_notes,o.created_at order_created_at,rt.name table_name,c.name customer_name,c.phone customer_phone FROM production_print_queue q JOIN production_stations ps ON ps.id=q.station_id AND ps.tenant_id=q.tenant_id JOIN orders o ON o.id=q.order_id AND o.tenant_id=q.tenant_id LEFT JOIN restaurant_tables rt ON rt.id=o.table_id LEFT JOIN customers c ON c.id=o.customer_id WHERE q.tenant_id=? AND q.unit_id=? AND ps.unit_id=? AND ps.active=1 AND q.status IN ("pending","error") AND q.attempts<5 AND (q.next_attempt_at IS NULL OR q.next_attempt_at<=CURRENT_TIMESTAMP) AND (ps.desktop_device_id IS NULL OR ps.desktop_device_id="" OR ps.desktop_device_id=?) ORDER BY q.created_at,q.id LIMIT 1 FOR UPDATE');
            $q=$pdo->prepare($sql);$q->execute([$tenantId,$unitId,$unitId,$deviceId]);$row=$q->fetch();if(!$row)return null;
            ...
            $items=$pdo->prepare('SELECT j.id,j.kind,j.description,j.quantity,j.status,j.prep_minutes,j.received_at FROM production_jobs j WHERE j.tenant_id=? AND j.order_id=? AND j.station_id=? AND j.status<>"cancelled" ORDER BY j.id');$items->execute([$tenantId,$row['order_id'],$row['station_id']]);$jobs=$items->fetchAll();if(!$jobs){$pdo->prepare('UPDATE production_print_queue SET status="failed",last_error="Nenhum item de produção encontrado.",failed_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?')->execute([$row['id'],$tenantId]);return null;}
            return['queue'=>$row,'items'=>$jobs,'printer_target'=>(string)($row['printer_target']??''),'device_id'=>$deviceId,'unit_id'=>$unitId];
        });
    }
    ...
    public function completePrint(int$queueId,string$deviceId,bool$success,string$error=''):array
    {
        ...
        return Database::transaction(function(PDO$pdo)use($tenantId,$userId,$queueId,$deviceId,$success,$error):array{
            $q=$pdo->prepare(Database::portableSql($pdo,'SELECT q.*,ps.name station_name,o.unit_id,o.status order_status FROM production_print_queue q JOIN production_stations ps ON ps.id=q.station_id AND ps.tenant_id=q.tenant_id JOIN orders o ON o.id=q.order_id AND o.tenant_id=q.tenant_id WHERE q.id=? AND q.tenant_id=? LIMIT 1 FOR UPDATE'));$q->execute([$queueId,$tenantId]);$row=$q->fetch();if(!$row)throw new RuntimeException('Item da fila de impressão não encontrado.');
            ...
            if($success){
                $pdo->prepare('UPDATE production_print_queue SET status="printed",printed_at=CURRENT_TIMESTAMP,last_error=NULL,next_attempt_at=NULL WHERE id=? AND tenant_id=?')->execute([$queueId,$tenantId]);
                $jobs=$pdo->prepare('SELECT id,description,quantity,status FROM production_jobs WHERE tenant_id=? AND order_id=? AND station_id=? AND status<>"cancelled" ORDER BY id');$jobs->execute([$tenantId,$row['order_id'],$row['station_id']]);$payloadHash=hash('sha256',json_encode($jobs->fetchAll(),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
                $ins=$pdo->prepare('INSERT INTO production_prints (tenant_id,unit_id,station_id,order_id,print_type,printed_by,reason,payload_hash) VALUES (?,?,?,?,"auto",?,?,?)');$ins->execute([$tenantId,$row['unit_id'],$row['station_id'],$row['order_id'],$userId,null,$payloadHash]);
                Auth::audit('production.desktop_printed','order',(string)$row['order_id'],['queue_id'=>$queueId,'station_id'=>(int)$row['station_id'],'device_id'=>$deviceId]);
            }else{
                ...
                if(Database::isSqlite($pdo)){$next=$terminal?null:gmdate('Y-m-d H:i:s',time()+$delay);$pdo->prepare('UPDATE production_print_queue SET status=?,last_error=?,failed_at=CURRENT_TIMESTAMP,next_attempt_at=?,claimed_at=NULL,claimed_device_id=NULL WHERE id=? AND tenant_id=?')->execute([$status,$error,$next,$queueId,$tenantId]);}
                else{$pdo->prepare('UPDATE production_print_queue SET status=?,last_error=?,failed_at=CURRENT_TIMESTAMP,next_attempt_at='.($terminal?'NULL':'DATE_ADD(CURRENT_TIMESTAMP,INTERVAL '.$delay.' SECOND)').',claimed_at=NULL,claimed_device_id=NULL WHERE id=? AND tenant_id=?')->execute([$status,$error,$queueId,$tenantId]);}
            ...
            $r=$pdo->prepare('SELECT * FROM production_print_queue WHERE id=? AND tenant_id=?');$r->execute([$queueId,$tenantId]);return$this->publicQueue($r->fetch()?:$row);
        });
    }
    ...
    private function publicQueue(array$row):array{unset($row['payload_hash']);return$row;}
}
