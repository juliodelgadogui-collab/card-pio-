...
class DeliveryApi(private val tokenProvider: () -> String?) {
    ...
    ) { customer(it.getJSONObject("customer")) }

    suspend fun saveAddress(address: Address): Address {
        val body = JSONObject()
            .put("id", address.id).put("label", address.label).put("street", address.street)
            .put("number", address.number).put("complement", address.complement)
            .put("neighborhood", address.neighborhood).put("city", address.city).put("state", address.state)
            .put("postal_code", address.postalCode).put("reference", address.reference).put("phone", address.phone)
            .put("is_default", address.isDefault)
        address.latitude?.let { body.put("latitude", it) }; address.longitude?.let { body.put("longitude", it) }
        return request("api-delivery-customer.php?action=address-save", "POST", body) { address(it.getJSONObject("address")) }
    }
    ...
    suspend fun cash(orderId: Int, changeForCents: Int?) { requestUnit(
        "api-delivery-customer.php?action=payment-cash", "POST",
        JSONObject().put("order_id", orderId).apply { if (changeForCents == null) put("change_for_cents", JSONObject.NULL) else put("change_for_cents", changeForCents) }
    ) }
    ...
    private fun <T> execute(path: String, method: String, body: JSONObject?, token: String?, parser: (JSONObject) -> T): T {
        val connection = (URL(base + path).openConnection() as HttpURLConnection).apply {
            requestMethod = method; connectTimeout = 10_000; readTimeout = 25_000; useCaches = false
