        ...
        try {
            ...
            $created = Database::transaction(function(PDO $tx) use($tenantName,$slug,$planCode,$planId,$businessType,$plansReady): array {
                ...
                $settings = json_encode([
                    ...
                    'points_enabled'=>true,
                    'menu_primary_color'=>'#f4b942',
                    'menu_background_color'=>'#0b0d12',
                    'menu_surface_color'=>'#141821',
                    'menu_text_color'=>'#f5f7fb',
                    'menu_layout'=>'cards',
...
<section class="page-hero platform-hero">
  <div><span class="eyebrow">EVENTMENU SAAS</span><h2>Controle comercial da plataforma</h2><p>Gerencie clientes, planos, preços e operação sem depender de telas técnicas.</p></div>
  <div class="hero-actions"><a class="button primary" href="<?= Security::e(app_url('?route=super&tab=companies#nova-empresa')) ?>">+ Nova empresa</a><a class="button secondary" href="<?= Security::e(app_url('?route=super&tab=plans')) ?>">Gerenciar planos</a></div>
</section>
<?php if(!$plansReady):?><div class="alert error"><strong>Atualização necessária.</strong> O módulo comercial ainda não existe neste banco. <a href="<?= Security::e(app_url('update.php')) ?>"><u>Aplicar atualização</u></a>.</div><?php endif;?>
...
<div class="grid commercial-grid">
  <section class="card"><span class="eyebrow">CARTEIRA</span><h2>Empresas recentes</h2><p class="muted">Acompanhe clientes e entre no painel quando precisar prestar suporte.</p><?php foreach(array_slice($tenants,0,6) as $t):?><div class="list-row"><div><strong><?= Security::e($t['name']) ?></strong><small><?= Security::e($t['slug']) ?> · <?= Security::e((string)($t['subscription_plan_name'] ?? $t['plan'])) ?></small></div><span class="status-pill <?= Security::e($t['status']) ?>"><?= Security::e($statusLabels[$t['status']] ?? $t['status']) ?></span></div><?php endforeach;?></section>
  <section class="card"><span class="eyebrow">PLANOS</span><h2>Portfólio comercial</h2><p class="muted">Os planos podem ter preço, benefícios e limites diferentes.</p><div class="plan-mini-grid"><?php foreach(array_slice($activePlans,0,3) as $p):?><article class="plan-mini"><strong><?= Security::e($p['name']) ?></strong><b><?= em_money($p['monthly_cents']) ?><small>/mês</small></b><span><?= Security::e($p['description'] ?? '') ?></span></article><?php endforeach;?></div><a class="button secondary" href="<?= Security::e(app_url('?route=super&tab=plans')) ?>">Editar planos</a></section>
</div>
<?php endif;?>

<?php if($tab === 'plans'):?>
<div class="grid admin-two-col">
<section class="card"><span class="eyebrow">PLANO COMERCIAL</span><h2><?= $editPlan?'Editar plano':'Criar novo plano' ?></h2><p class="muted">Defina valor, benefícios e limites. Um plano arquivado continua funcionando para quem já o utiliza.</p>
