...
final class PaymentService
{
    ...
    public function confirmVerified(array $verified):void
    {
        ...
        Database::transaction(function(PDO $pdo)use($verified):void{
            ...
            $payment=$this->findLocalPayment($pdo,$tenantId,$orderId,$provider,$verified);
            ...
            if($paidAfter<$total){
                ...
                Auth::audit('payment.partial_confirmed','payment',(string)$payment['id'],['order_id'=>$orderId,'paid_cents'=>$paidAfter,'remaining_cents'=>$total-$paidAfter]);return;
            }
    ...
    private function findLocalPayment(PDO $pdo,int $tenantId,int $orderId,string $provider,array $verified):array|false
    {
        ...
        $providerId=(string)($verified['provider_payment_id']??'');
        if($providerId!==''){$s=$pdo->prepare(Database::portableSql($pdo,'SELECT * FROM payments WHERE tenant_id=? AND order_id=? AND provider=? AND provider_payment_id=? ORDER BY id DESC LIMIT 1 FOR UPDATE'));$s->execute([$tenantId,$orderId,$provider,$providerId]);if($p=$s->fetch())return $p;}
    ...
    {
        $settlement='settlement:order:'.$orderId;
        (new StockReservationService())->consumeForSettlement($pdo,$tenantId,$orderId,$settlement);
        ...
        if(!empty($order['coupon_id'])){
            $r=$pdo->prepare(Database::portableSql($pdo,'SELECT * FROM coupon_reservations WHERE tenant_id=? AND order_id=? AND status="reserved" FOR UPDATE'));$r->execute([$tenantId,$orderId]);$reservation=$r->fetch();
            $insertSql=Database::portableSql($pdo,'INSERT IGNORE INTO coupon_redemptions (tenant_id,coupon_id,order_id,customer_id,discount_cents,idempotency_key) VALUES (?,?,?,?,?,?)');
