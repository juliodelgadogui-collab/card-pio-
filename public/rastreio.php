...
$initial = (new DeliveryTrackingService())->latestPublic($token);
...
let marker=null,trail=L.polyline([],{weight:5,opacity:.75,lineCap:'round'}).addTo(map),follow=true,firstFix=true,pollTimer=null,animation=null;
...
function apply(x){
 ...
 if(x.order_status==='completed'){$('headline').textContent='Pedido entregue ✓';$('subline').textContent='Entrega concluída. Obrigado por acompanhar pelo EventMenu.';chip.className='chip';if(pollTimer){clearInterval(pollTimer);pollTimer=null}}
 else if(sig.status==='offline'){$('headline').textContent='Seu pedido continua em rota';$('subline').textContent='O sinal do entregador está temporariamente indisponível. Mostramos a última posição conhecida.'}
