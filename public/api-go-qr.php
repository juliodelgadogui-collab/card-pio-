...
try{
    ...
    $service=new UniversalQrService();$limit=new SensitiveApiRateLimitService();$action=(string)($_GET['action']??'resolve');

    if($action==='resolve'){
        $value=(string)($_GET['value']??'');goqr_out(['ok'=>true,'qr'=>$service->resolve($value)]);
    }
    if($action==='issue'){
        if($_SERVER['REQUEST_METHOD']!=='POST')goqr_out(['ok'=>false,'error'=>'Método não permitido.'],405);$limit->assertAllowed('qr.issue',$user,$deviceId);$body=goqr_body();
    ...
    if($action==='revoke'){
        if($_SERVER['REQUEST_METHOD']!=='POST')goqr_out(['ok'=>false,'error'=>'Método não permitido.'],405);$limit->assertAllowed('qr.revoke',$user,$deviceId);$body=goqr_body();$service->revoke((string)($body['type']??''),(int)($body['entity_id']??0));goqr_out(['ok'=>true]);
    }
    goqr_out(['ok'=>false,'error'=>'Endpoint de QR não encontrado.'],404);
}catch(ApiRateLimitExceededException $e){goqr_out(['ok'=>false,'error'=>$e->getMessage()],429);}catch(RuntimeException $e){goqr_out(['ok'=>false,'error'=>$e->getMessage()],422);}catch(Throwable $e){if(filter_var(env('APP_DEBUG','false'),FILTER_VALIDATE_BOOL))goqr_out(['ok'=>false,'error'=>$e->getMessage()],500);goqr_out(['ok'=>false,'error'=>'Erro interno.'],500);}
