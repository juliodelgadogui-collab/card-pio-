...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        if($unitId){$scope=$pdo->prepare('SELECT id FROM orders WHERE id=? AND tenant_id=? AND unit_id=? LIMIT 1');$scope->execute([$id,$tenantId,$unitId]);if(!$scope->fetchColumn())throw new RuntimeException('Este pedido não pertence à unidade selecionada.');}
        (new OrderWorkflowService())->transition($tenantId,$id,$status);em_flash('ok','Pedido #'.$id.' atualizado.');
    }catch(Throwable $e){em_flash('error',$e->getMessage());}em_go('kitchen');
}

$sql='SELECT o.id,o.channel,o.status,o.created_at,o.notes,o.scheduled_for,rt.name table_name,c.name customer_name,(SELECT GROUP_CONCAT(CONCAT(oi.quantity,"× ",oi.name_snapshot) ORDER BY oi.id SEPARATOR " | ") FROM order_items oi WHERE oi.order_id=o.id) items FROM orders o LEFT JOIN restaurant_tables rt ON rt.id=o.table_id LEFT JOIN customers c ON c.id=o.customer_id WHERE o.tenant_id=?'.($unitId?' AND o.unit_id=?':'').' AND o.status IN ("confirmed","preparing","ready") AND (o.status IN ("preparing","ready") OR o.scheduled_for IS NULL OR o.scheduled_for<=DATE_ADD(UTC_TIMESTAMP(),INTERVAL '.$lead.' MINUTE)) ORDER BY FIELD(o.status,"preparing","confirmed","ready"),COALESCE(o.scheduled_for,o.created_at),o.id';
$s=$pdo->prepare($sql);$args=[$tenantId];if($unitId)$args[]=$unitId;$s->execute($args);$orders=$s->fetchAll();
