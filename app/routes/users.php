...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='save'){
        ...
        $commissionPercent=(float)str_replace(',','.',trim((string)($_POST['delivery_commission_percent']??'0')));$commissionPercent=max(0,min(100,$commissionPercent));$commissionBps=(int)round($commissionPercent*100);
        $commissionFixedValue=(float)str_replace(',','.',trim((string)($_POST['delivery_commission_fixed']??'0')));$commissionFixedCents=max(0,min(100000000,(int)round($commissionFixedValue*100)));
        ...
        try{
            Database::transaction(function(PDO $tx)use(&$id,$tenantId,$name,$email,$phone,$role,$status,$password,$selected,$selectedUnits,$defaultUnitId,$commissionBps,$commissionFixedCents):void{
                if($id){
                    ...
                    if($password!==''&&strlen($password)<6)throw new RuntimeException('A senha precisa ter pelo menos 6 caracteres.');
                    if($password!==''){$s=$tx->prepare('UPDATE users SET name=?,email=?,phone=?,role=?,status=?,password_hash=?,delivery_commission_bps=?,delivery_commission_fixed_cents=? WHERE id=? AND tenant_id=?');$s->execute([$name,$email,$phone?:null,$role,$status,password_hash($password,PASSWORD_DEFAULT),$commissionBps,$commissionFixedCents,$id,$tenantId]);}
                    else{$s=$tx->prepare('UPDATE users SET name=?,email=?,phone=?,role=?,status=?,delivery_commission_bps=?,delivery_commission_fixed_cents=? WHERE id=? AND tenant_id=?');$s->execute([$name,$email,$phone?:null,$role,$status,$commissionBps,$commissionFixedCents,$id,$tenantId]);}
                }else{
                    if(strlen($password)<6)throw new RuntimeException('A senha precisa ter pelo menos 6 caracteres.');
                    $s=$tx->prepare('INSERT INTO users (tenant_id,name,email,phone,password_hash,role,status,delivery_commission_bps,delivery_commission_fixed_cents) VALUES (?,?,?,?,?,?,?,?,?)');$s->execute([$tenantId,$name,$email,$phone?:null,password_hash($password,PASSWORD_DEFAULT),$role,$status,$commissionBps,$commissionFixedCents]);$id=(int)$tx->lastInsertId();
                }
            ...
            Auth::audit('user.saved','user',(string)$id,['role'=>$role,'status'=>$status,'effective_permissions'=>$selected,'unit_ids'=>$selectedUnits,'default_unit_id'=>$defaultUnitId?:null,'delivery_commission_bps'=>$commissionBps,'delivery_commission_fixed_cents'=>$commissionFixedCents]);
            em_flash('ok',$status==='blocked'?'Usuário bloqueado; app, refresh, NFC e turno foram encerrados.':'Usuário, permissões, unidades e comissão salvos.');
...
?><div class="grid" style="grid-template-columns:minmax(330px,1fr) minmax(0,2fr)"><section class="card"><h2><?= $edit?'Editar usuário':'Novo usuário' ?></h2><form method="post" class="form-grid" id="user-form"><input type="hidden" name="_csrf" value="<?= em_csrf() ?>"><input type="hidden" name="action" value="save"><input type="hidden" name="id" value="<?= (int)($edit['id']??0) ?>"><label class="span-2">Nome<input name="name" required value="<?= Security::e($edit['name']??'') ?>"></label><label class="span-2">E-mail<input type="email" name="email" required value="<?= Security::e($edit['email']??'') ?>"></label><label class="span-2">Telefone<input name="phone" value="<?= Security::e($edit['phone']??'') ?>"></label><label>Função-base<select name="role" id="user-role"><?php foreach($roles as $r):?><option value="<?= $r ?>"<?= em_selected($edit['role']??'waiter',$r) ?>><?= Security::e($roleLabels[$r]??$r) ?></option><?php endforeach;?></select></label><label>Status<select name="status"><option value="active"<?= em_selected($edit['status']??'active','active') ?>>Ativo</option><option value="blocked"<?= em_selected($edit['status']??'','blocked') ?>>Bloqueado</option></select></label><label class="span-2"><?= $edit?'Nova senha (deixe vazio para manter)':'Senha' ?><input type="password" name="password" minlength="6"<?= $edit?'':' required' ?>><small>Mínimo de 6 caracteres.</small></label>
<div class="span-2"><h3>Comissão Delivery</h3><p class="muted">Calculada somente sobre entregas pagas e concluídas no turno deste funcionário. É possível combinar percentual e valor fixo.</p><div class="grid" style="grid-template-columns:1fr 1fr;gap:10px"><label>Percentual por entrega (%)<input type="number" name="delivery_commission_percent" min="0" max="100" step="0.01" value="<?= Security::e(number_format(((int)($edit['delivery_commission_bps']??0))/100,2,'.','')) ?>"></label><label>Valor fixo por entrega (R$)<input type="number" name="delivery_commission_fixed" min="0" step="0.01" value="<?= Security::e(number_format(((int)($edit['delivery_commission_fixed_cents']??0))/100,2,'.','')) ?>"></label></div></div>
...
<div class="span-2"><h3>Permissões efetivas</h3><p class="muted">O cargo-base é apenas um preset. Marque exatamente o que este funcionário poderá fazer no painel e no EventMenu GO.</p><div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:8px"><?php foreach($permissionLabels as $permission=>$label):?><label class="checkbox"><input class="perm-check" type="checkbox" name="permissions[]" value="<?= Security::e($permission) ?>"<?= in_array($permission,$selectedPermissions,true)?' checked':'' ?>> <?= Security::e($label) ?></label><?php endforeach;?></div><button type="button" class="secondary" id="apply-role" style="margin-top:10px">Aplicar preset da função</button></div><button class="primary span-2">Salvar usuário</button><?php if($edit):?><a class="button secondary span-2" href="<?= Security::e(app_url('?route=users')) ?>">Cancelar</a><?php endif;?></form><p class="muted">Alterar permissão, unidade ou comissão tem efeito no servidor. O funcionário não consegue liberar funções ou editar sua comissão modificando o aplicativo.</p></section><section class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Usuário</th><th>Função-base</th><th>Unidades</th><th>Comissão Delivery</th><th>Status</th><th>Turno</th><th>App</th><th>NFC</th><th></th></tr></thead><tbody><?php foreach($users as $u):$pct=((int)($u['delivery_commission_bps']??0))/100;$fixed=(int)($u['delivery_commission_fixed_cents']??0);?><tr><td><strong><?= Security::e($u['name']) ?></strong><br><span class="muted"><?= Security::e($u['email']) ?></span></td><td><span class="badge"><?= Security::e($roleLabels[$u['role']]??$u['role']) ?></span></td><td><?= Security::e(isset($unitMap[(int)$u['id']])?implode(', ',$unitMap[(int)$u['id']]):'Todas') ?></td><td><?= $pct>0?Security::e(number_format($pct,2,',','.')).'%':'0%' ?><?= $fixed>0?' + '.em_money($fixed).'/entrega':'' ?></td><td><?= Security::e($u['status']) ?></td><td><?= (int)$u['open_shifts']?'Aberto':'Fechado' ?></td><td><?= (int)$u['active_app_tokens'] ?></td><td><?= (int)$u['active_devices'] ?></td><td><a class="button secondary" href="<?= Security::e(app_url('?route=users&edit='.(int)$u['id'])) ?>">Editar</a></td></tr><?php endforeach;?></tbody></table></div></section></div>
