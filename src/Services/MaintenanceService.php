...
final class MaintenanceService
{
    public function run():array
    {
        ...
        try{$whatsapp=new WhatsAppIntegrationService();$result['whatsapp_payment_events']=$whatsapp->syncPaymentEvents($pdo,max(1,min(500,(int)env('WHATSAPP_EVENT_SYNC_BATCH',100))));$result['whatsapp_outbox']=$whatsapp->processOutbox($pdo,max(1,min(100,(int)env('WHATSAPP_OUTBOX_BATCH',20))));}catch(\Throwable$e){$result['whatsapp_outbox']=['error'=>mb_substr($e->getMessage(),0,240)];}
