...
$scheduleStmt=$pdo->prepare('SELECT starts_at,ends_at,title,description,location FROM event_schedule_items WHERE tenant_id=? AND event_id=? AND active=1 ORDER BY starts_at,sort_order,id');
$scheduleStmt->execute([(int)$event['tenant_id'],(int)$event['id']]);
...
<nav class="nav"><div class="nav-links"><a href="#ingressos">Ingressos</a><?php if($scheduleItems):?><a href="#programacao">Programação</a><?php endif;?><?php if($barOrdering):?><a href="<?= Security::e($barUrl) ?>">Pedir no bar</a><?php endif;?><a href="#local">Local</a><a href="#experiencia">Experiência</a></div><a class="cta" href="#ingressos">Comprar ingresso →</a></nav>
<?php if($error):?><div class="alert"><?= Security::e($error) ?></div><?php endif;?>

<?php if($scheduleItems):?>
<section class="card schedule" id="programacao">
  <div class="section-head"><div><span class="eyebrow" style="color:var(--primary)">PROGRAMAÇÃO</span><h2>Confira os horários</h2></div><span class="muted">Programação sujeita a ajustes pela organização</span></div>
  <div class="schedule-list"><?php foreach($scheduleItems as$item):?><article class="schedule-item"><div class="schedule-time"><?= Security::e(date('H:i',strtotime((string)$item['starts_at']))) ?></div><div><h3><?= Security::e($item['title']) ?></h3><?php if($item['location']):?><div class="schedule-place"><?= Security::e($item['location']) ?></div><?php endif;?><?php if($item['description']):?><p><?= nl2br(Security::e($item['description'])) ?></p><?php endif;?><?php if($item['ends_at']):?><small class="muted">Até <?= Security::e(date('H:i',strtotime((string)$item['ends_at']))) ?></small><?php endif;?></div></article><?php endforeach;?></div>
</section>
<?php endif;?>
