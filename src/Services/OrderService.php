...
final class OrderService
{
    ...
    public function changeStatus(int $orderId,string $target,string $source='panel'):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$orderId,$target,$source):array{
            ...
            if($source==='delivery'){if(!in_array($target,['out_for_delivery','completed'],true))throw new RuntimeException('A entrega só pode retirar ou concluir o pedido.');if(Auth::role()==='delivery'&&(int)($order['assigned_delivery_user_id']??0)!==(int)Auth::id())throw new RuntimeException('Pedido não está atribuído a este entregador.');if($order['channel']!=='delivery')throw new RuntimeException('Pedido não é de delivery.');}
            ...
            $pdo->prepare('UPDATE orders SET status=? WHERE id=? AND tenant_id=?')->execute([$target,$orderId,$tenantId]);Auth::audit('order.status','order',(string)$orderId,['from'=>$current,'to'=>$target,'source'=>$source]);$order['status']=$target;return $order;
        });
    }
}
