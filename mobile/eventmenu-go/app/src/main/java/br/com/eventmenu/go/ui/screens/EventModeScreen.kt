...
fun EventModeScreen(events:List<EventOverview>,selectedEventId:Int?,entries:List<EventEntry>,canScanTicket:Boolean,canScanGuest:Boolean,canUseBar:Boolean,onSelect:(Int)->Unit,onOpenBar:(Int)->Unit,onRefresh:()->Unit,onScan:()->Unit){
 ...
 LazyColumn(Modifier.fillMaxSize().padding(14.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  item{Text("Ingressos",style=MaterialTheme.typography.headlineMedium,fontWeight=FontWeight.Black);Text("Bilheteria presencial integrada às vendas e à capacidade dos eventos.",color=MaterialTheme.colorScheme.onSurfaceVariant)}
  item{Card(Modifier.fillMaxWidth(),colors=CardDefaults.cardColors(containerColor=MaterialTheme.colorScheme.primaryContainer)){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(10.dp)){Text("Bilheteria",style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.Black);Text("Venda avulsa ou identificada. Cada ingresso recebe seu próprio QR Code.");Button(onClick={context.startActivity(Intent(context,TicketSalesActivity::class.java))},modifier=Modifier.fillMaxWidth().heightIn(min=56.dp)){Text("🎟 VENDER INGRESSO")};if(canScanTicket||canScanGuest)OutlinedButton(onClick=onScan,modifier=Modifier.fillMaxWidth().heightIn(min=52.dp)){Text(if(canScanTicket&&canScanGuest)"📷 LER INGRESSO / CONVIDADO" else if(canScanTicket)"📷 LER INGRESSO" else "📷 LER CONVIDADO")}}}}
  item{Text("Eventos e vendas",style=MaterialTheme.typography.titleLarge,fontWeight=FontWeight.Black)}
  ...
  if(selected==null)item{Text("Nenhum evento disponível para operação neste turno.")}else{
   ...
   item{Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){EventMetric("Vendidos",selected.ticketsPaid.toString(),Modifier.weight(1f));EventMetric("Check-ins",selected.ticketsCheckedIn.toString(),Modifier.weight(1f))}}
   ...
   item{Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(8.dp)){EventMetric("Bilheteria",eventMoney(selected.ticketRevenueCents),Modifier.weight(1f));EventMetric("Bar",eventMoney(selected.barRevenueCents),Modifier.weight(1f))}}
...
@Composable private fun EventEntryCard(entry:EventEntry){Card(Modifier.fillMaxWidth()){Column(Modifier.padding(14.dp),verticalArrangement=Arrangement.spacedBy(4.dp)){Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){Text(if(entry.type=="ticket")"Ingresso" else "Convidado",fontWeight=FontWeight.Bold);Text(friendlyEventDate(entry.checkedInAt),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)};if(entry.personName.isNotBlank())Text(entry.personName,fontWeight=FontWeight.SemiBold);if(entry.detail.isNotBlank())Text(entry.detail,color=MaterialTheme.colorScheme.onSurfaceVariant);if(entry.operatorName.isNotBlank())Text("Entrada por ${entry.operatorName}",style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)}}}
