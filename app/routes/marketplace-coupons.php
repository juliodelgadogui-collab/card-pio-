...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if($action==='toggle'){
            ...
            $result=Database::transaction(function(PDO$tx)use($id,$active,$service):array{$tx->prepare('UPDATE marketplace_delivery_coupons SET active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')->execute([$active,$id]);return$service->syncCoupon($tx,$id);});
