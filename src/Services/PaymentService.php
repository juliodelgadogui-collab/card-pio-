...
final class PaymentService
{
    ...
    {
        $required = ['tenant_id','order_id','provider','provider_payment_id','amount_cents','currency','account_reference'];
        ...
        Database::transaction(function (PDO $pdo) use ($verified) {
            ...
            foreach ($items->fetchAll() as $item) {
                ...
                $pdo->prepare('INSERT INTO stock_movements (tenant_id,product_id,order_id,type,quantity,idempotency_key) VALUES (?,?,?,"out",?,?)')
                    ->execute([$tenantId, $item['product_id'], $orderId, $qty, $key]);
            }
            ...
            foreach ($tickets->fetchAll() as $row) {
                $qty = (int)$row['qty'];
                $batch = $pdo->prepare('UPDATE ticket_batches SET quantity_reserved=GREATEST(0,quantity_reserved-?),quantity_sold=quantity_sold+? WHERE id=?');
                $batch->execute([$qty,$qty,$row['batch_id']]);
            }
            ...
            if (!empty($order['promoter_id'])) {
                ...
                if ($percent !== false) {
                    ...
                    $pdo->prepare('INSERT IGNORE INTO promoter_commissions (tenant_id,promoter_id,order_id,amount_cents,status,idempotency_key) VALUES (?,?,?,?,"approved",?)')
                        ->execute([$tenantId,$order['promoter_id'],$orderId,$commission,'payment:'.$payment['id'].':commission']);
                }
            }
        });
    }
