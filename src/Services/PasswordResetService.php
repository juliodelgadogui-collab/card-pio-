...
final class PasswordResetService
{
    public function request(string $email): bool
    {
        ...
        $html = '<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;color:#1f2430"><h2 style="color:#4d2db7">Redefinição de senha</h2><p>Olá, '.$safeName.'. Recebemos um pedido para alterar sua senha do EventMenu.</p><p><a href="'.$safeUrl.'" style="display:inline-block;padding:12px 18px;background:#4d2db7;color:#fff;text-decoration:none;border-radius:9px">Criar nova senha</a></p><p>Este link expira em <strong>30 minutos</strong> e só pode ser usado uma vez.</p><p style="color:#687083;font-size:13px">Se você não solicitou a alteração, ignore este e-mail. Sua senha atual continuará válida.</p></div>';
    ...
    public function reset(string $token, string $password): void
    {
        ...
        if (strlen($password) < 1 || strlen($password) > 200) throw new RuntimeException('Senha inválida.');
        $hash = hash('sha256', $token);
        Database::transaction(function (PDO $pdo) use ($hash, $password): void {
            ...
            try {$pdo->prepare('INSERT INTO audit_logs (tenant_id,user_id,action,entity_type,entity_id,ip_address,user_agent,metadata) VALUES (?,?,?,?,?,?,?,?)')->execute([$row['tenant_id'] !== null ? (int)$row['tenant_id'] : null,$userId,'auth.password_reset','user',(string)$userId,Security::clientIp(),mb_substr((string)($_SERVER['HTTP_USER_AGENT']??''),0,500),json_encode(['method'=>'email_token'],JSON_UNESCAPED_UNICODE)]);} catch (\Throwable) {}
        });
    }
