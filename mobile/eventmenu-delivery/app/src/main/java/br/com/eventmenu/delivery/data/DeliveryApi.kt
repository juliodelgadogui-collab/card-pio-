...
private fun categoryNames(array:JSONArray?):List<String>{if(array==null)return emptyList();val out=linkedSetOf<String>();for(i in 0 until array.length()){val name=when(val value=array.opt(i)){is JSONObject->value.optString("name");is String->value;else->""}.trim();if(name.isNotBlank())out+=name};return out.toList()}
