...
final class MaintenanceService
{
    public function releaseExpiredReservations():array
    {
        return Database::transaction(function(PDO $pdo){
            $releasedTickets=0;$releasedCoupons=0;$cancelledOrders=0;$throttlesCleaned=0;
            ...
            foreach($groups as $g){$qty=(int)$g['qty'];$pdo->prepare('UPDATE ticket_batches SET quantity_reserved=GREATEST(0,quantity_reserved-?) WHERE id=?')->execute([$qty,$g['batch_id']]);$releasedTickets+=$qty;}
            ...
            foreach($couponRows as $row){$pdo->prepare('UPDATE coupon_reservations SET status="released" WHERE id=?')->execute([$row['id']]);$pdo->prepare('UPDATE coupons SET reserved_count=GREATEST(0,reserved_count-1) WHERE id=?')->execute([$row['coupon_id']]);$releasedCoupons++;}
            ...
            try{
                ...
                $cleanup->execute();$throttlesCleaned=$cleanup->rowCount();
            }catch(\Throwable){}

            return ['tickets_released'=>$releasedTickets,'coupons_released'=>$releasedCoupons,'orders_cancelled'=>$cancelledOrders,'login_throttles_cleaned'=>$throttlesCleaned];
        });
    }
}
