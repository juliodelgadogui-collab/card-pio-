...
try{
    ...
    if($action==='qr-resolve'){
        ...
        if(Auth::can('tables.manage')||Auth::can('orders.create')){$s=$pdo->prepare('SELECT rt.id,rt.name,rt.seats,rt.status,rt.qr_token,t.id tab_id,t.label tab_label FROM restaurant_tables rt LEFT JOIN tabs t ON t.table_id=rt.id AND t.tenant_id=rt.tenant_id AND t.status="open" WHERE rt.tenant_id=? AND rt.qr_token=? LIMIT 1');$s->execute([$tenantId,$value]);if($row=$s->fetch())api_out(['ok'=>true,'type'=>'table','data'=>$row]);}
        if(Auth::can('tickets.manage')){$s=$pdo->prepare('SELECT t.id,t.code,t.status,t.event_id,e.name event_name,e.status event_status,c.name customer_name FROM tickets t JOIN events e ON e.id=t.event_id AND e.tenant_id=t.tenant_id LEFT JOIN customers c ON c.id=t.customer_id AND c.tenant_id=t.tenant_id WHERE t.tenant_id=? AND (t.qr_token=? OR t.code=?) LIMIT 1');$s->execute([$tenantId,$value,$value]);if($row=$s->fetch())api_out(['ok'=>true,'type'=>'ticket','data'=>$row]);}
        if(Auth::can('guests.manage')){$s=$pdo->prepare('SELECT g.id,g.name,g.status,g.plus_ones,g.event_id,e.name event_name,e.status event_status,g.checkin_code FROM event_guests g JOIN events e ON e.id=g.event_id AND e.tenant_id=g.tenant_id WHERE g.tenant_id=? AND g.checkin_code=? LIMIT 1');$s->execute([$tenantId,$value]);if($row=$s->fetch())api_out(['ok'=>true,'type'=>'guest','data'=>$row]);}
        api_out(['ok'=>false,'error'=>'QR/código não reconhecido para sua função.'],404);
    }
    if($action==='ticket-checkin'){
        api_method('POST');if(!Auth::can('tickets.manage'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$body=api_body();$eventId=(int)($body['event_id']??0);if($eventId<1)api_out(['ok'=>false,'error'=>'Selecione o evento antes de validar a entrada.'],422);$result=(new TicketService())->checkIn((string)($body['token']??''),$eventId);api_out(['ok'=>true,'result'=>$result]);
    }
    if($action==='guest-checkin'){
        api_method('POST');if(!Auth::can('guests.manage'))api_out(['ok'=>false,'error'=>'Acesso negado.'],403);$body=api_body();$eventId=(int)($body['event_id']??0);if($eventId<1)api_out(['ok'=>false,'error'=>'Selecione o evento antes de validar o convidado.'],422);$guest=(new GuestService())->checkIn((string)($body['code']??''),$eventId);api_out(['ok'=>true,'guest'=>$guest]);
    }
