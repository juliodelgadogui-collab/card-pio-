...
final class MarketplaceCatalogService
{
    public function stores(PDO $pdo, array $filters = []): array
    {
        ...
        $sql = 'SELECT t.id tenant_id,t.name,t.slug,t.settings,ms.city,ms.state,ou.id unit_id,ou.code unit_code,ou.name unit_name,ou.address unit_address FROM marketplace_tenant_settings ms JOIN tenants t ON t.id=ms.tenant_id JOIN operating_units ou ON ou.tenant_id=t.id AND ou.active=1 WHERE ms.participates=1 AND ms.status="active" AND t.status="active"';
        $args = [];
        ...
        foreach ($s->fetchAll() as $row) {
            ...
            $rows[] = [
                ...
                'address'=>(string)($row['unit_address'] ?? ''),
                'logo_url'=>$this->publicUrl($settings['menu_logo_url'] ?? ''),
    ...
    public function catalog(PDO $pdo, int $tenantId, int $unitId): array
    {
        ...
        $p=$pdo->prepare('SELECT p.id,p.category_id,p.name,p.description,p.price_cents,p.image_url,p.track_stock,COALESCE(ui.stock_qty,0) unit_stock_qty,EXISTS(SELECT 1 FROM product_recipes r WHERE r.tenant_id=p.tenant_id AND r.product_id=p.id) has_recipe FROM products p LEFT JOIN unit_inventory ui ON ui.tenant_id=p.tenant_id AND ui.unit_id=? AND ui.product_id=p.id WHERE p.tenant_id=? AND p.active=1 ORDER BY p.category_id,p.name');$p->execute([$unitId,$tenantId]);$products=$p->fetchAll();
        $modifiers=(new ConfiguredOrderService())->catalogModifiers($pdo,$tenantId,array_column($products,'id'));
        $safeProducts=[];
        foreach($products as$product){
            $available=!(int)$product['track_stock']||(int)$product['has_recipe']||(float)$product['unit_stock_qty']>0;
            $groups=[];
            foreach($modifiers[(int)$product['id']]??[] as$group){
                $opts=[];foreach((array)($group['options']??[])as$option){$opts[]=['id'=>(int)$option['id'],'name'=>(string)$option['name'],'price_delta_cents'=>(int)$option['price_delta_cents']];}
                $min=max((int)($group['min_select']??0),(int)($group['required']??0)?1:0);$max=max(1,(int)($group['max_select']??1));
                $groups[]=['id'=>(int)$group['id'],'name'=>(string)$group['name'],'required'=>$min>0,'min_select'=>$min,'max_select'=>$max,'options'=>$opts];
            }
        ...
        return [
            'store'=>[
                'tenant_id'=>$tenantId,'unit_id'=>$unitId,'slug'=>(string)$row['slug'],
                'name'=>trim((string)($settings['menu_public_title']??''))?:(string)$row['name'],
                'description'=>(string)($settings['menu_subtitle']??''),'unit_name'=>(string)$unitRow['name'],
                'city'=>(string)($market['city']??''),'state'=>(string)($market['state']??''),'address'=>(string)($unitRow['address']??''),
                'logo_url'=>$this->publicUrl($settings['menu_logo_url']??''),'cover_url'=>$this->publicUrl($settings['menu_cover_url']??''),
                'delivery_fee_cents'=>max(0,(int)($settings['delivery_fee_cents']??0)),'minimum_order_cents'=>max(0,(int)($settings['min_delivery_order_cents']??0)),
            ],
    ...
    private function publicUrl(mixed $value): string
    {
        ...
        return \app_absolute_url(ltrim($url,'/'));
    }
}
