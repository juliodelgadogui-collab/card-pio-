...
{
    public function createIntent(int $orderId,string $deviceIdentifier):array
    {
        ...
        $pdo=Database::connection();$hash=hash('sha256',$deviceIdentifier);
        $d=$pdo->prepare('SELECT * FROM nfc_devices WHERE tenant_id=? AND device_identifier_hash=? AND status="active" LIMIT 1');$d->execute([$tenantId,$hash]);$device=$d->fetch();
        if(!$device) throw new RuntimeException('Aparelho NFC não está autorizado.');
        if($device['user_id']!==null&&(int)$device['user_id']!==$userId) throw new RuntimeException('Aparelho NFC está vinculado a outro usuário.');

        $o=$pdo->prepare('SELECT * FROM orders WHERE id=? AND tenant_id=? LIMIT 1');$o->execute([$orderId,$tenantId]);$order=$o->fetch();if(!$order)throw new RuntimeException('Pedido não encontrado.');
        if(in_array($order['status'],['cancelled','completed'],true)||$order['payment_status']==='paid')throw new RuntimeException('Pedido não aceita cobrança.');
        if(Auth::role()==='delivery'&&(int)($order['assigned_delivery_user_id']??0)!==$userId)throw new RuntimeException('Este pedido não está atribuído a você.');
        $amount=(int)$order['total_cents'];if($amount<100||$amount>1000000)throw new RuntimeException('Valor fora do limite permitido pelo Tap On.');
        ...
        $pdo->prepare('INSERT INTO nfc_payment_intents (tenant_id,order_id,user_id,nfc_device_id,intent_token_hash,amount_cents,status,expires_at) VALUES (?,?,?,?,?, ?,"created",?)')->execute([$tenantId,$orderId,$userId,$device['id'],$tokenHash,$amount,$expires]);
        $intentId=(int)$pdo->lastInsertId();Auth::audit('nfc.intent_created','order',(string)$orderId,['intent_id'=>$intentId,'device_id'=>(int)$device['id'],'amount_cents'=>$amount]);
        return ['intent_token'=>$raw,'order_id'=>$orderId,'amount_cents'=>$amount,'sale_amount'=>$amount/100,'expires_at'=>$expires,'tap_on'=>['app_key'=>$appKey,'app_name'=>(string)($config['tap_on_app_name']??'EventMenu'),'app_version'=>(string)($config['tap_on_app_version']??'1.0.0'),'enable_tax_pass_through'=>(bool)($config['tap_on_tax_pass_through']??false)]];
    }
    ...
    public function verifyIntent(string $intentToken,string $transactionCode,string $deviceIdentifier):array
    {
        ...
        $pdo=Database::connection();$hash=hash('sha256',$intentToken);$stmt=$pdo->prepare('SELECT i.*,d.device_identifier_hash,d.status device_status,d.user_id device_user_id,o.status order_status,o.payment_status order_payment_status FROM nfc_payment_intents i JOIN nfc_devices d ON d.id=i.nfc_device_id JOIN orders o ON o.id=i.order_id WHERE i.tenant_id=? AND i.intent_token_hash=? LIMIT 1');$stmt->execute([$tenantId,$hash]);$intent=$stmt->fetch();
        if(!$intent||(int)$intent['user_id']!==$userId)throw new RuntimeException('Intenção NFC inválida.');
        if($intent['device_status']!=='active'||!hash_equals((string)$intent['device_identifier_hash'],hash('sha256',$deviceIdentifier)))throw new RuntimeException('Aparelho NFC não autorizado.');
        if($intent['device_user_id']!==null&&(int)$intent['device_user_id']!==$userId)throw new RuntimeException('Aparelho NFC pertence a outro usuário.');
        ...
        if(in_array($intent['order_status'],['cancelled','completed'],true)||$intent['order_payment_status']==='paid')throw new RuntimeException('Pedido não aceita confirmação NFC.');

        $provider=$this->queryTapOnTransaction($tenantId,$transactionCode);
        if(!$provider['paid'])throw new RuntimeException('PagBank ainda não confirmou esta transação.');
        if((int)$provider['amount_cents']!==(int)$intent['amount_cents'])throw new RuntimeException('Valor da transação PagBank divergente.');
        ...
        (new PaymentService())->confirmVerified(['tenant_id'=>$tenantId,'order_id'=>(int)$intent['order_id'],'provider'=>'pagbank','provider_payment_id'=>$transactionCode,'amount_cents'=>(int)$provider['amount_cents'],'currency'=>'BRL','account_reference'=>(string)$provider['account_reference'],'raw_status'=>(string)$provider['raw_status'],'source'=>'tap_on']);
        $pdo->prepare('UPDATE nfc_payment_intents SET status="verified",payment_id=?,verified_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?')->execute([$paymentId,$intent['id'],$tenantId]);Auth::audit('nfc.payment_verified','order',(string)$intent['order_id'],['intent_id'=>(int)$intent['id'],'payment_id'=>$paymentId,'transaction_code'=>$transactionCode]);
        return ['ok'=>true,'order_id'=>(int)$intent['order_id'],'payment_id'=>$paymentId,'transaction_code'=>$transactionCode];
    }
