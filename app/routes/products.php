...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='product-save'){
        $id=(int)($_POST['id']??0);$name=trim((string)($_POST['name']??''));$sku=trim((string)($_POST['sku']??''));$price=(int)round((float)str_replace(',','.',(string)($_POST['price']??'0'))*100);$cat=(int)($_POST['category_id']??0);$track=isset($_POST['track_stock'])?1:0;$description=trim((string)($_POST['description']??''));$image=trim((string)($_POST['image_url']??''))?:null;$active=isset($_POST['active'])?1:0;
        ...
        if($id){
            $exists=$pdo->prepare('SELECT id,stock_qty FROM products WHERE id=? AND tenant_id=?');$exists->execute([$id,$tenantId]);if(!$exists->fetch())exit('Produto não encontrado.');
            ...
            $stock=(float)str_replace(',','.',(string)($_POST['stock_qty']??'0'));if(!is_finite($stock)||$stock<0)exit('Estoque inicial inválido.');
            ...
            $s->execute([$tenantId,$cat?:null,$name,$description,$sku?:null,$price,$stock,$track,$image,$active]);$id=(int)$pdo->lastInsertId();
            if($track&&$stock>0){$key='initial-stock:'.$tenantId.':'.$id;$pdo->prepare('INSERT INTO stock_movements (tenant_id,product_id,order_id,type,quantity,idempotency_key) VALUES (?,?,NULL,"in",?,?)')->execute([$tenantId,$id,$stock,$key]);}
        }
...
?><div class="grid" style="grid-template-columns:minmax(290px,1fr) minmax(0,2fr)"><section class="card"><h2><?= $edit?'Editar produto':'Novo produto' ?></h2><form method="post" class="form-grid"><input type="hidden" name="_csrf" value="<?= em_csrf() ?>"><input type="hidden" name="action" value="product-save"><input type="hidden" name="id" value="<?= (int)($edit['id']??0) ?>"><label class="span-2">Nome<input name="name" required value="<?= Security::e($edit['name']??'') ?>"></label><label>Preço<input name="price" required inputmode="decimal" value="<?= $edit?Security::e(number_format(((int)$edit['price_cents'])/100,2,',','')):'' ?>"></label><label>SKU<input name="sku" value="<?= Security::e($edit['sku']??'') ?>"></label><label class="span-2">Categoria<select name="category_id"><option value="0">Sem categoria</option><?php foreach($categories as $c):?><option value="<?= (int)$c['id'] ?>"<?= em_selected($edit['category_id']??0,$c['id']) ?>><?= Security::e($c['name']) ?></option><?php endforeach;?></select></label><?php if($edit):?><label>Estoque atual<input value="<?= Security::e((string)($edit['stock_qty']??'0')) ?>" readonly></label><div><span class="muted">O saldo é alterado somente pelo módulo Estoque.</span><br><?php if(Auth::can('inventory.manage')):?><a class="button secondary" style="margin-top:8px" href="<?= Security::e(app_url('?route=inventory&product_id='.(int)$edit['id'])) ?>">Movimentar estoque</a><?php endif;?></div><?php else:?><label>Estoque inicial<input name="stock_qty" inputmode="decimal" value="0"></label><div class="muted">Após cadastrar, novas alterações ficam registradas no módulo Estoque.</div><?php endif;?><label class="checkbox span-2"><input type="checkbox" name="track_stock"<?= em_checked($edit['track_stock']??0) ?>> Controlar estoque</label><label class="span-2">Imagem (URL)<input type="url" name="image_url" value="<?= Security::e($edit['image_url']??'') ?>"></label><label class="span-2">Descrição<textarea name="description"><?= Security::e($edit['description']??'') ?></textarea></label><label class="checkbox span-2"><input type="checkbox" name="active"<?= em_checked($edit?($edit['active']??0):1) ?>> Produto ativo</label><button class="primary span-2">Salvar produto</button><?php if($edit):?><a class="button secondary span-2" href="<?= Security::e(app_url('?route=products')) ?>">Cancelar edição</a><?php endif;?></form><hr style="border-color:var(--line);margin:24px 0"><h3>Nova categoria</h3><form method="post"><input type="hidden" name="_csrf" value="<?= em_csrf() ?>"><input type="hidden" name="action" value="category-save"><label>Nome<input name="name" required></label><label>Ordem<input type="number" name="sort_order" value="0"></label><button class="secondary" style="margin-top:10px">Criar categoria</button></form></section><section class="card"><div class="section-head"><h2>Produtos</h2><span class="muted"><?= count($products) ?> cadastrados</span></div><div class="table-wrap"><table class="table"><thead><tr><th>Produto</th><th>Categoria</th><th>Preço</th><th>Estoque</th><th>Status</th><th></th></tr></thead><tbody><?php foreach($products as $p):?><tr><td><strong><?= Security::e($p['name']) ?></strong><br><span class="muted"><?= Security::e($p['sku']??'') ?></span></td><td><?= Security::e($p['category_name']??'—') ?></td><td><?= em_money($p['price_cents']) ?></td><td><?= $p['track_stock']?Security::e((string)$p['stock_qty']):'Livre' ?></td><td><span class="badge"><?= $p['active']?'Ativo':'Inativo' ?></span></td><td><div class="actions"><a class="button secondary" href="<?= Security::e(app_url('?route=products&edit='.(int)$p['id'])) ?>">Editar</a><?php if($p['track_stock']&&Auth::can('inventory.manage')):?><a class="button secondary" href="<?= Security::e(app_url('?route=inventory&product_id='.(int)$p['id'])) ?>">Estoque</a><?php endif;?></div></td></tr><?php endforeach;?></tbody></table></div></section></div><?php em_footer();
