...
// Delivery: cobre o caso real em que um turno legado/flexível não possui unit_id,
// mas o pedido atribuído pertence a uma unidade operacional válida.
$deliveryUnitCode='ops-delivery-unit-'.bin2hex(random_bytes(3));$pdo->prepare('INSERT INTO operating_units (tenant_id,code,name,active) VALUES (?,?,"Delivery CI",1)')->execute([$tenantId,$deliveryUnitCode]);$deliveryUnitId=(int)$pdo->lastInsertId();
$paid=bin2hex(random_bytes(20));$pdo->prepare('INSERT INTO orders (public_token,tenant_id,unit_id,assigned_delivery_user_id,channel,status,payment_status,subtotal_cents,total_cents,created_by) VALUES (?, ?, ?, ?, "delivery", "ready", "paid", 2000, 2000, ?)')->execute([$paid,$tenantId,$deliveryUnitId,$deliveryId,$adminId]);$deliveryOrder=(int)$pdo->lastInsertId();
...
$pdo->prepare('INSERT INTO work_shifts (tenant_id,user_id,unit_id,mode,status,opening_notes) VALUES (?,?,NULL,"delivery","open","CI turno flexível")')->execute([$tenantId,$deliveryId]);
$deliveryProgress=new DeliveryProgressService();$mine=$deliveryProgress->listMine();ops_assert(count(array_filter($mine,fn(array$row):bool=>(int)$row['order_id']===$deliveryOrder))===1,'Pedido atribuído desapareceu de Minhas entregas quando o turno não tinha unidade.');
$picked=$deliveryProgress->pickup($deliveryOrder);ops_assert(!empty($picked['picked_up_at']),'Retirada do pedido não foi registrada.');
$routed=$deliveryProgress->startRoute($deliveryOrder);ops_assert(!empty($routed['route_started_at']),'Início da rota não foi registrado.');
$arrived=$deliveryProgress->arrive($deliveryOrder);ops_assert(!empty($arrived['arrived_at']),'Chegada ao cliente não foi registrada.');
$completed=$deliveryProgress->complete($deliveryOrder);ops_assert(($completed['order']['status']??'')==='completed','Entrega não foi concluída pelo fluxo de progresso.');
$status=$pdo->query('SELECT status FROM orders WHERE id='.(int)$deliveryOrder)->fetchColumn();ops_assert($status==='completed','Status final do pedido Delivery divergiu.');
$pdo->prepare('UPDATE work_shifts SET status="closed",ended_at=CURRENT_TIMESTAMP WHERE tenant_id=? AND user_id=? AND status="open"')->execute([$tenantId,$deliveryId]);
...
$cash=new CashService();$cashSession=$cash->open(10000,'CI abertura');ops_assert((int)$cashSession['opening_cash_cents']===10000,'Abertura do caixa falhou.');ops_assert((int)$cashSession['unit_id']===$panelUnitId,'Caixa não abriu na unidade selecionada.');try{$cash->open(100);ops_fail('Segundo caixa foi aberto para o mesmo operador.');}catch(RuntimeException){}
$cash->addManualMovement('supply',2000,'Suprimento CI');$cash->addManualMovement('withdrawal',1000,'Sangria CI');$cash->addManualMovement('adjustment',500,'Ajuste CI','out');
$cashToken=bin2hex(random_bytes(20));$pdo->prepare('INSERT INTO orders (public_token,tenant_id,unit_id,channel,status,payment_status,subtotal_cents,total_cents,created_by) VALUES (?, ?, ?, "counter", "confirmed", "paid", 3000, 3000, ?)')->execute([$cashToken,$tenantId,$panelUnitId,$adminId]);$cashOrder=(int)$pdo->lastInsertId();$providerPaymentId='CASH-CI-'.strtoupper(bin2hex(random_bytes(6)));$paymentKey='cash-ci-'.$uid;$pdo->prepare('INSERT INTO payments (tenant_id,order_id,provider,provider_payment_id,idempotency_key,amount_cents,currency,status,verified_at) VALUES (?, ?, "manual", ?, ?, 3000, "BRL", "paid", CURRENT_TIMESTAMP)')->execute([$tenantId,$cashOrder,$providerPaymentId,$paymentKey]);$paymentId=(int)$pdo->lastInsertId();$cash->recordPaidPayment($paymentId,'cash');$cash->recordPaidPayment($paymentId,'cash');
$cashSummary=$cash->summary((int)$cashSession['id']);ops_assert((int)$cashSummary['expected_cash_cents']===13500,'Saldo esperado do caixa divergente.');$recorded=0;foreach($cashSummary['movements'] as $movement){if((int)($movement['payment_id']??0)===$paymentId)$recorded++;}ops_assert($recorded===1,'Pagamento entrou mais de uma vez no caixa.');$closed=$cash->close(13400,'CI fechamento');ops_assert((int)$closed['expected_cash_cents']===13500,'Fechamento calculou esperado incorreto.');ops_assert((int)$closed['difference_cents']===-100,'Diferença do caixa incorreta.');try{$cash->addManualMovement('supply',100,'Após fechar');ops_fail('Caixa fechado aceitou movimento.');}catch(RuntimeException){}
