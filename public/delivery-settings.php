...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if($gateway)$pdo->prepare('UPDATE payment_gateways SET account_reference=?,config_encrypted=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?')->execute([$account?:null,$encrypted,$active,(int)$gateway['id'],$tenantId]);
        else{$pdo->prepare("INSERT INTO payment_gateways (tenant_id,provider,account_reference,config_encrypted,active) VALUES (?,'mercadopago',?,?,?)")->execute([$tenantId,$account?:null,$encrypted,$active]);$gateway=['id'=>(int)$pdo->lastInsertId(),'account_reference'=>$account,'active'=>$active];}
        $gateway['account_reference']=$account;$gateway['active']=$active;
        Auth::audit('delivery.settings_saved','tenant',(string)$tenantId,['delivery_paused'=>$settings['delivery_paused'],'cash'=>$settings['delivery_cash_enabled'],'pix'=>$mp['pix_enabled'],'card'=>$mp['card_enabled'],'mercadopago_account'=>$account!==''?'configured':'missing']);
