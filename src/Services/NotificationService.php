...
final class NotificationService
{
    ...
    public function publishToUser(
        ...
        $tenantId=Auth::tenantId();if(!$tenantId||$userId<1)throw new RuntimeException('Destino de notificação inválido.');
    ...
    public function publishToPermission(
        string $permission,
        ?string $mode,
        string $type,
        string $title,
    ...
    public function inbox(int $limit=100): array
    {
        ...
        $s=$pdo->prepare($sql);$s->execute($args);$items=$s->fetchAll();
        $unread=0;foreach($items as $item)if(empty($item['read_at']))$unread++;
    ...
    private function insert(PDO $pdo,int $tenantId,int $userId,?string $mode,string $type,string $title,string $message,?string $entityType,?string $entityId,?string $dedupeKey,string $priority,?string $expiresAt): bool
    {
        ...
        $dedupeKey=mb_substr(trim($dedupeKey?:('notification:'.bin2hex(random_bytes(16)))),0,190);
        if($expiresAt!==null&&strtotime($expiresAt)===false)$expiresAt=null;
