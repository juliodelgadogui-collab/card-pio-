...
final class TicketService
{
    public function reservePublic(int $eventId, int $batchId, int $quantity, array $buyer, ?string $couponCode = null, ?string $promoterCode = null): array
    {
        ...
        return Database::transaction(function (PDO $pdo) use ($eventId,$batchId,$quantity,$buyer,$couponCode,$promoterCode) {
            $stmt = $pdo->prepare('SELECT e.*,t.status tenant_status FROM events e JOIN tenants t ON t.id=e.tenant_id WHERE e.id=? AND e.status="published" FOR UPDATE');
            $stmt->execute([$eventId]);
            $event = $stmt->fetch();
            if (!$event || $event['tenant_status'] !== 'active') throw new RuntimeException('Evento indisponível.');

            $stmt = $pdo->prepare('SELECT * FROM ticket_batches WHERE id=? AND event_id=? AND active=1 FOR UPDATE');
            ...
            if ($couponCode) {
                ...
                $c=$pdo->prepare('SELECT * FROM coupons WHERE tenant_id=? AND code=? AND active=1 FOR UPDATE');$c->execute([$event['tenant_id'],$code]);$coupon=$c->fetch();
                ...
                if ($coupon['max_uses'] !== null && (int)$coupon['uses_count'] >= (int)$coupon['max_uses']) throw new RuntimeException('Limite do cupom atingido.');
                if ($subtotal < (int)$coupon['min_order_cents']) throw new RuntimeException('Valor mínimo do cupom não atingido.');
                $couponId=(int)$coupon['id'];
                $discount=$coupon['type']==='percent' ? (int)round($subtotal*min(100,(int)$coupon['value'])/100) : min($subtotal,(int)$coupon['value']);
            }

            $promoterId = null;
            if ($promoterCode) { $p=$pdo->prepare('SELECT id FROM promoters WHERE tenant_id=? AND code=? AND active=1');$p->execute([$event['tenant_id'],strtoupper(trim($promoterCode))]);$promoterId=$p->fetchColumn() ?: null; }
    ...
    public function checkIn(string $token): array
    {
        ...
        return Database::transaction(function(PDO $pdo) use($tenantId,$token){
            $s=$pdo->prepare('SELECT * FROM tickets WHERE tenant_id=? AND (qr_token=? OR code=?) LIMIT 1 FOR UPDATE');$s->execute([$tenantId,$token,$token]);$ticket=$s->fetch();
            ...
            $pdo->prepare('UPDATE tickets SET status="checked_in",checked_in_at=NOW(),checked_in_by=? WHERE id=?')->execute([Auth::id(),$ticket['id']]);
    ...
    public function releaseExpired(): int
    {
        return Database::transaction(function(PDO $pdo){
            $s=$pdo->query('SELECT batch_id,COUNT(*) qty FROM tickets WHERE status="reserved" AND reserved_until IS NOT NULL AND reserved_until<NOW() GROUP BY batch_id FOR UPDATE');
            $groups=$s->fetchAll();$total=0;
            foreach($groups as $g){$pdo->prepare('UPDATE ticket_batches SET quantity_reserved=GREATEST(0,quantity_reserved-?) WHERE id=?')->execute([(int)$g['qty'],$g['batch_id']]);$total+=(int)$g['qty'];}
            $pdo->exec('UPDATE tickets SET status="cancelled" WHERE status="reserved" AND reserved_until IS NOT NULL AND reserved_until<NOW()');
            $pdo->exec('UPDATE orders o SET o.status="cancelled",o.payment_status=IF(o.payment_status="unpaid","failed",o.payment_status) WHERE o.channel="event" AND o.payment_status<>"paid" AND NOT EXISTS (SELECT 1 FROM tickets t WHERE t.order_id=o.id AND t.status="reserved") AND EXISTS (SELECT 1 FROM tickets t2 WHERE t2.order_id=o.id)');
            return $total;
        });
    }
