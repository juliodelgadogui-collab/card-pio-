...
final class DeliveryCashService
{
    public function collect(int $orderId,int $receivedCents):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$userId,$orderId,$receivedCents,$shift):array{
            ...
            (new PaymentService())->confirmVerified(['payment_id'=>$paymentId,'tenant_id'=>$tenantId,'order_id'=>$orderId,'provider'=>'manual','provider_payment_id'=>'DELIVERY-CASH-'.$orderId,'amount_cents'=>$total,'currency'=>'BRL','account_reference'=>'manual','source'=>'delivery_cash']);
