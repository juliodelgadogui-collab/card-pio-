...
try{
    $auth=new ApiAuthService();$token=ApiAuthService::bearerToken();$deviceId=ApiAuthService::deviceId();if($token==='')goc_out(['ok'=>false,'error'=>'Token Bearer obrigatório.'],401);$auth->authenticate($token,$deviceId);
    $action=(string)($_GET['action']??'pending');$service=new OrderCancellationService();
    ...
    if($action==='request')goc_out(['ok'=>true,'request'=>$service->request((int)($body['order_id']??0),(string)($body['reason']??''))],201);
    if($action==='approve')goc_out(['ok'=>true,'result'=>$service->approve((int)($body['request_id']??0))]);
    if($action==='reject')goc_out(['ok'=>true,'result'=>$service->reject((int)($body['request_id']??0),(string)($body['reason']??''))]);
    goc_out(['ok'=>false,'error'=>'Endpoint de cancelamento não encontrado.'],404);
}catch(RuntimeException $e){goc_out(['ok'=>false,'error'=>$e->getMessage()],422);}catch(Throwable $e){if(filter_var(env('APP_DEBUG','false'),FILTER_VALIDATE_BOOL))goc_out(['ok'=>false,'error'=>$e->getMessage()],500);goc_out(['ok'=>false,'error'=>'Erro interno.'],500);}
