...
final class OrderCreationService
{
    public function create(array $payload):array
    {
        ...
        if($channel==='bar'){
            ...
            $shift=(new WorkShiftService())->current();if(!$shift||$shift['mode']!=='events')throw new RuntimeException('Venda de bar só pode ser lançada durante turno de Eventos.');
        ...
        $order=Database::transaction(function(PDO $tx)use($tenantId,$userId,$channel,$tableId,$eventId,$name,$phone,$address,$notes,$quantities):array{
            $validTableId=null;$tabId=null;$validEventId=null;
            if($channel==='table'){
            ...
            $token=bin2hex(random_bytes(20));$o=$tx->prepare('INSERT INTO orders (public_token,tenant_id,customer_id,channel,status,payment_status,subtotal_cents,total_cents,table_id,tab_id,event_id,delivery_address,notes,created_by) VALUES (?,?,?,?,?,"unpaid",?,?,?,?,?,?,?,?)');$o->execute([$token,$tenantId,$customerId,$channel,$initialStatus,$total,$total,$validTableId,$tabId,$validEventId,$channel==='delivery'?$address:null,$notes?:null,$userId]);$orderId=(int)$tx->lastInsertId();
            ...
            Auth::audit('order.created_staff','order',(string)$orderId,['channel'=>$channel,'event_id'=>$validEventId,'total_cents'=>$total,'items'=>count($items)]);
            return ['id'=>$orderId,'public_token'=>$token,'channel'=>$channel,'status'=>$initialStatus,'payment_status'=>'unpaid','total_cents'=>$total,'customer_id'=>$customerId,'table_id'=>$validTableId,'tab_id'=>$tabId,'event_id'=>$validEventId];
        });
