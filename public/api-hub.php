...
try{
    ...
    if($action==='command-create'){
        hub_method('POST');$body=hub_body();$device=hub_device($body,$sessionDevice);$payload=is_array($body['payload']??null)?$body['payload']:[];
    ...
    if($action==='desktop-complete'){
        hub_method('POST');$body=hub_body();$device=hub_device($body,$sessionDevice);$result=is_array($body['result']??null)?$body['result']:[];
    ...
    if($action==='link-revoke'){
        hub_method('POST');$body=hub_body();hub_device($body,$sessionDevice);$hub->revokeLink((int)($body['id']??0));hub_out(['ok'=>true]);
    }
