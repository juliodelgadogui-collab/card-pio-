...
$found=$identity->findByPhone($pdo,$tenantId,'+55 '.$normalized);customer_ci_assert(is_array($found)&&(int)$found['id']===(int)$first['id'],'Busca por variação do telefone não encontrou o mesmo cliente.');
$reused=$identity->findOrCreate($pdo,$tenantId,'Cliente Atualizado','+55 '.$normalized,'Rua Teste, 10');customer_ci_assert((int)$reused['id']===(int)$first['id'],'findOrCreate criou cliente duplicado.');

$duplicateBlocked=false;try{$identity->save($pdo,$tenantId,['name'=>'Duplicado','phone'=>'+55 '.$normalized]);}catch(RuntimeException){$duplicateBlocked=true;}customer_ci_assert($duplicateBlocked,'Cadastro duplicado por telefone não foi bloqueado.');
