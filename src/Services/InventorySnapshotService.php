...
final class InventorySnapshotService
{
    public function snapshot(bool $lowOnly=false):array
    {
        ...
        $summaryStmt->execute([$tenantId,$unitId,$unitId,$tenantId]);
        $summary=$summaryStmt->fetch()?:['controlled'=>0,'low'=>0,'zero'=>0,'reserved_qty'=>0];
        ...
        foreach($rows as&$row){
            ...
            $minimum=(float)$row['min_stock_qty'];
            $row['physical_qty']=$available+(float)$row['reserved_qty'];
            $row['stock_status']=$available<=0?'zero':($available<=$minimum?'low':'ok');
        }
        unset($row);

        return [
            'unit'=>['id'=>$unitId,'name'=>(string)$unit['name']],
            'summary'=>[
                'controlled'=>(int)$summary['controlled'],
                'low'=>(int)$summary['low'],
                'zero'=>(int)$summary['zero'],
                'reserved_qty'=>(float)$summary['reserved_qty'],
            ],
            'products'=>$rows,
        ];
    }
