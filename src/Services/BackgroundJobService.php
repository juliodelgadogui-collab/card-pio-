...
final class BackgroundJobService
{
    ...
    {
        $limit=max(1,min(100,$limit));$workerId=mb_substr(trim((string)$workerId),0,100)?:('php-'.getmypid().'-'.substr(bin2hex(random_bytes(5)),0,10));$result=['processed'=>0,'completed'=>0,'retried'=>0,'failed'=>0,'worker'=>$workerId];
        for($i=0;$i<$limit;$i++){
    ...
    {
        $pdo=Database::connection();$stats=['pending'=>0,'processing'=>0,'retry'=>0,'failed'=>0,'completed_recent'=>0,'oldest_pending_at'=>null];
    ...
    private function handle(array $job):void
    {
        ...
        match((string)$job['type']){
            ...
            'backup.daily'=>(new BackupService())->runScheduled(),
