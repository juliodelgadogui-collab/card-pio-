...
final class CheckoutService
{
    public function create(string $publicToken,string $provider):array
    {
        ...
        if(!$order||$order['tenant_status']!=='active')throw new RuntimeException('Pedido não encontrado.');if(in_array($order['status'],['cancelled','completed'],true)||$order['payment_status']==='paid')throw new RuntimeException('Pedido não aceita nova cobrança.');if((int)$order['total_cents']<=0)throw new RuntimeException('Pedido com valor inválido.');
        ...
        if($order['channel']==='event'){$r=$pdo->prepare('SELECT MIN(reserved_until) FROM tickets WHERE order_id=? AND status="reserved"');$r->execute([$order['id']]);$until=$r->fetchColumn();if($until&&new \DateTimeImmutable()>new \DateTimeImmutable((string)$until))throw new RuntimeException('A reserva dos ingressos expirou. Faça uma nova reserva.');}
        ...
        try{$result=match($provider){'stripe'=>$this->stripe($order,$gateway,$config,$key),'mercadopago'=>$this->mercadoPago($order,$gateway,$config,$key),'pagbank'=>$this->pagBank($order,$gateway,$config,$key)};$payload=$result['raw'];$payload['_eventmenu_checkout_url']=$result['url'];$pdo->prepare('UPDATE payments SET provider_payment_id=?,status="pending",raw_payload=? WHERE id=?')->execute([$result['external_id'],json_encode($payload,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),$paymentId]);return ['provider'=>$provider,'url'=>$result['url'],'payment_id'=>$paymentId,'reused'=>false];}catch(\Throwable $e){$pdo->prepare('UPDATE payments SET status="failed",raw_payload=? WHERE id=?')->execute([json_encode(['error'=>$e->getMessage()],JSON_UNESCAPED_UNICODE),$paymentId]);$pdo->prepare('UPDATE orders SET payment_status="failed" WHERE id=? AND payment_status="pending"')->execute([$order['id']]);throw $e;}
    }
    ...
    private function stripe(array $order,array $gateway,array $config,string $key):array
    {
        ...
        $session=$client->checkout->sessions->create(['mode'=>'payment','line_items'=>[['price_data'=>['currency'=>'brl','unit_amount'=>(int)$order['total_cents'],'product_data'=>['name'=>'Pedido EventMenu #'.$order['id']]],'quantity'=>1]],'client_reference_id'=>(string)$order['id'],'customer_email'=>$order['customer_email']?:null,'metadata'=>$metadata,'payment_intent_data'=>['metadata'=>$metadata],'success_url'=>$base.'/pedido.php?t='.rawurlencode($order['public_token']).'&retorno=sucesso','cancel_url'=>$base.'/pedido.php?t='.rawurlencode($order['public_token']).'&retorno=cancelado'],['idempotency_key'=>$key]);return ['url'=>(string)$session->url,'external_id'=>(string)$session->id,'raw'=>$session->toArray()];
    }

    private function mercadoPago(array $order,array $gateway,array $config,string $key):array
    {
        $token=(string)($config['access_token']??'');if($token==='')throw new RuntimeException('Mercado Pago não configurado.');$base=rtrim((string)env('APP_URL',''),'/');$notification=$base.'/webhook.php?provider=mercadopago&tenant='.rawurlencode($order['tenant_slug']);$body=['items'=>[['id'=>'order-'.$order['id'],'title'=>'Pedido EventMenu #'.$order['id'],'quantity'=>1,'currency_id'=>'BRL','unit_price'=>((int)$order['total_cents'])/100]],'external_reference'=>'eventmenu:'.$order['tenant_id'].':'.$order['id'],'back_urls'=>['success'=>$base.'/pedido.php?t='.rawurlencode($order['public_token']),'pending'=>$base.'/pedido.php?t='.rawurlencode($order['public_token']),'failure'=>$base.'/pedido.php?t='.rawurlencode($order['public_token'])],'notification_url'=>$notification,'metadata'=>['tenant_id'=>$order['tenant_id'],'order_id'=>$order['id']]];$data=$this->httpJson('POST','https://api.mercadopago.com/checkout/preferences',['Authorization: Bearer '.$token,'X-Idempotency-Key: '.$key],$body);$url=(string)($data['init_point']??'');if($url==='')throw new RuntimeException('Mercado Pago não retornou URL de pagamento.');return ['url'=>$url,'external_id'=>(string)($data['id']??''),'raw'=>$data];
    }

    private function pagBank(array $order,array $gateway,array $config,string $key):array
    {
        $token=(string)($config['token']??'');if($token==='')throw new RuntimeException('PagBank não configurado.');$api=rtrim((string)($config['api_base']??'https://api.pagseguro.com'),'/');$base=rtrim((string)env('APP_URL',''),'/');$notification=$base.'/webhook.php?provider=pagbank&tenant='.rawurlencode($order['tenant_slug']);$body=['reference_id'=>'eventmenu:'.$order['tenant_id'].':'.$order['id'],'items'=>[['reference_id'=>'order-'.$order['id'],'name'=>'Pedido EventMenu #'.$order['id'],'quantity'=>1,'unit_amount'=>(int)$order['total_cents']]],'payment_methods'=>[['type'=>'CREDIT_CARD'],['type'=>'PIX']],'redirect_url'=>$base.'/pedido.php?t='.rawurlencode($order['public_token']),'return_url'=>$base.'/pedido.php?t='.rawurlencode($order['public_token']),'redirect_waiting_time'=>5,'notification_urls'=>[$notification],'payment_notification_urls'=>[$notification]];$data=$this->httpJson('POST',$api.'/checkouts',['Authorization: Bearer '.$token,'x-idempotency-key: '.$key],$body);$url='';foreach(($data['links']??[])as$link){if(($link['rel']??'')==='PAY'){$url=(string)$link['href'];break;}}if($url==='')throw new RuntimeException('PagBank não retornou link de pagamento.');return ['url'=>$url,'external_id'=>(string)($data['id']??''),'raw'=>$data];
    }
