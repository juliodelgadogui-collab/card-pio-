    ...
    if(!Security::validateCsrf($_POST['_csrf']??null)){
        ...
        try{
            if($action==='add'){
                ...
                $p=$pdo->prepare('SELECT id FROM products WHERE id=? AND tenant_id=? AND active=1');
                $p->execute([$id,$tenantId]);
                if(!$p->fetchColumn())throw new RuntimeException('Produto indisponível.');
                $cart[$id]=min(50,(int)($cart[$id]??0)+1);
                header('Location: /menu.php?empresa='.urlencode($slug).'#carrinho');exit;
            }
            if($action==='qty'){
                $id=(int)($_POST['product_id']??0);
                $qty=max(0,min(50,(int)($_POST['qty']??0)));
                if($qty===0)unset($cart[$id]);else$cart[$id]=$qty;
                header('Location: /menu.php?empresa='.urlencode($slug).'#carrinho');exit;
            }
            if($action==='clear'){
                $cart=[];
                header('Location: /menu.php?empresa='.urlencode($slug));exit;
            }
            if($action==='checkout'){
                $payload=[];
                foreach($cart as$id=>$qty)$payload[]=['product_id'=>(int)$id,'qty'=>(int)$qty];
                $buyer=[
                ...
                $result=(new PublicOrderService())->createDelivery($tenantId,$payload,$buyer,(string)($_POST['address']??''),(string)($_POST['coupon']??''));
                $cart=[];
                header('Location: /pedido.php?t='.urlencode($result['public_token']),true,303);exit;
            }
