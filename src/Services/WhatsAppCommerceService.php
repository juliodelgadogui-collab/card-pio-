...
final class WhatsAppCommerceService
{
    ...
    public function receiveInbound(string $deviceId,array $input):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$deviceHash,$providerId,$phone,$name,$type,$text,$payload,$providerCreatedAt):array{
            ...
            if(in_array($normalized,self::MENU_COMMANDS,true)||$assistant->isGreeting($text)||($state==='IDLE'&&trim($text)==='')){
                ...
                $reply=$assistant->welcome($assistantSettings,$name,$this->tenantName($pdo,$tenantId));$queued=$this->queueReply($pdo,$tenantId,$conversationId,$phone,$providerId,'welcome',$reply);
                return ['duplicate'=>false,'conversation_id'=>$conversationId,'mode'=>'auto','state'=>'WELCOME','reply_queued'=>$queued,'commerce_enabled'=>true,'assistant'=>true];
            }

            if($state==='IDLE'){
                $knowledge=$assistant->answer($pdo,$tenantId,$text);
                if($knowledge!==null){$pdo->prepare("UPDATE whatsapp_conversations SET state='WELCOME',last_activity_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?")->execute([$conversationId,$tenantId]);$reply=trim((string)$knowledge['answer'])."\n\nDigite *MENU* para fazer um pedido ou *ATENDENTE* para falar com uma pessoa.";$queued=$this->queueReply($pdo,$tenantId,$conversationId,$phone,$providerId,'knowledge_'.$knowledge['id'],$reply);return ['duplicate'=>false,'conversation_id'=>$conversationId,'mode'=>'auto','state'=>'WELCOME','reply_queued'=>$queued,'commerce_enabled'=>true,'assistant'=>true,'knowledge_id'=>$knowledge['id']];}
                if((int)($assistantSettings['enabled']??0)===1&&$assistant->unknownBehavior($assistantSettings)==='human')return$this->transferToHuman($pdo,$tenantId,$conversationId,$phone,$providerId,$assistantSettings);
            ...
            if($state==='WELCOME'){
                $knowledge=$assistant->answer($pdo,$tenantId,$text);
                if($knowledge!==null){$reply=trim((string)$knowledge['answer'])."\n\nDigite *MENU* para voltar às opções ou *ATENDENTE* para falar com uma pessoa.";$queued=$this->queueReply($pdo,$tenantId,$conversationId,$phone,$providerId,'knowledge_'.$knowledge['id'],$reply);return ['duplicate'=>false,'conversation_id'=>$conversationId,'mode'=>'auto','state'=>$state,'reply_queued'=>$queued,'commerce_enabled'=>true,'assistant'=>true,'knowledge_id'=>$knowledge['id']];}
                if((int)($assistantSettings['enabled']??0)===1&&$assistant->unknownBehavior($assistantSettings)==='human')return$this->transferToHuman($pdo,$tenantId,$conversationId,$phone,$providerId,$assistantSettings);
                $queued=$this->queueReply($pdo,$tenantId,$conversationId,$phone,$providerId,'assistant_unknown',$assistant->unknown($assistantSettings));return ['duplicate'=>false,'conversation_id'=>$conversationId,'mode'=>'auto','state'=>$state,'reply_queued'=>$queued,'commerce_enabled'=>true,'assistant'=>true,'unknown'=>true];
            }
    ...
    {if($tenantId<1||$conversationId<1)throw new RuntimeException('Conversa inválida.');if(!in_array($mode,['human','auto'],true))throw new RuntimeException('Modo de atendimento inválido.');return Database::transaction(function(PDO $tx)use($tenantId,$conversationId,$mode,$userId):array{$sql=Database::portableSql($tx,'SELECT * FROM whatsapp_conversations WHERE id=? AND tenant_id=? LIMIT 1 FOR UPDATE');$q=$tx->prepare($sql);$q->execute([$conversationId,$tenantId]);$row=$q->fetch(PDO::FETCH_ASSOC);if(!$row)throw new RuntimeException('Conversa não encontrada.');if($mode==='human'){if(!$userId)throw new RuntimeException('Usuário responsável inválido.');$tx->prepare("UPDATE whatsapp_conversations SET mode='human',state='HUMAN',assigned_user_id=?,last_activity_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?")->execute([$userId,$conversationId,$tenantId]);}else{$next=(int)($row['draft_order_id']??0)>0?'CART':'WELCOME';$tx->prepare("UPDATE whatsapp_conversations SET mode='auto',state=?,context_json=NULL,assigned_user_id=NULL,last_activity_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?")->execute([$next,$conversationId,$tenantId]);}return ['id'=>$conversationId,'mode'=>$mode,'previous_mode'=>(string)$row['mode']];});}

    /** @param array<string,mixed>|null $assistantSettings @return array<string,mixed> */
    private function transferToHuman(PDO $pdo,int $tenantId,int $conversationId,string $phone,string $providerId,?array $assistantSettings=null):array
    {$assistant=new WhatsAppCommerceAssistantService();$settings=$assistantSettings??$assistant->settings($pdo,$tenantId);$pdo->prepare("UPDATE whatsapp_conversations SET mode='waiting_human',state='WAITING_HUMAN',assigned_user_id=NULL,last_activity_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND tenant_id=?")->execute([$conversationId,$tenantId]);$replyQueued=$this->queueReply($pdo,$tenantId,$conversationId,$phone,$providerId,'human_transfer',$assistant->handoff($settings));try{Auth::audit('whatsapp.conversation_waiting_human','whatsapp_conversation',(string)$conversationId,['phone_suffix'=>substr($phone,-4)]);}catch(Throwable){}return ['duplicate'=>false,'conversation_id'=>$conversationId,'mode'=>'waiting_human','state'=>'WAITING_HUMAN','reply_queued'=>$replyQueued,'commerce_enabled'=>true];}

    private function auditStage1Transition(PDO $pdo,int $tenantId,array $conversation,array $result):void
    {try{$conversationId=(int)($conversation['id']??0);$from=(string)($conversation['state']??'IDLE');$to=(string)($result['state']??$from);$oldDraftId=(int)($conversation['draft_order_id']??0);$newDraftId=(int)($result['draft_order_id']??0);$draftId=$newDraftId>0?$newDraftId:$oldDraftId;$kind=(string)($result['kind']??'');$context=[];$raw=$conversation['context_json']??null;if(is_string($raw)&&trim($raw)!==''){$decoded=json_decode($raw,true);if(is_array($decoded))$context=$decoded;}if((int)($conversation['customer_id']??0)<1&&$conversationId>0){$q=$pdo->prepare('SELECT customer_id FROM whatsapp_conversations WHERE id=? AND tenant_id=?');$q->execute([$conversationId,$tenantId]);$customerId=(int)($q->fetchColumn()?:0);if($customerId>0)Auth::audit('whatsapp.customer_identified','customer',(string)$customerId,['conversation_id'=>$conversationId]);}$event='';if($from==='WELCOME'&&$to==='SELECTING_CATEGORY'&&$draftId>0)$event='whatsapp.draft_created';elseif($from==='ENTERING_NOTE'&&$to==='CART'&&$draftId>0)$event='whatsapp.item_added';elseif($from==='CART'&&in_array($to,['UPSELL','CHOOSING_FULFILLMENT'],true)&&$draftId>0)$event='whatsapp.cart_finalized';elseif($kind==='cart_cancelled')$event='whatsapp.draft_cancelled';elseif($from==='CART'&&$to==='CART'&&($context['cart_action']??'')==='qty_value')$event='whatsapp.item_quantity_changed';elseif($from==='CART'&&$to==='CART'&&($context['cart_action']??'')==='remove_item')$event='whatsapp.item_removed';if($event!=='')Auth::audit($event,$draftId>0?'order':'whatsapp_conversation',(string)($draftId>0?$draftId:$conversationId),['conversation_id'=>$conversationId,'from_state'=>$from,'to_state'=>$to]);}catch(Throwable){}}
    ...
    private function tenantName(PDO $pdo,int $tenantId):string
    {try{$q=$pdo->prepare('SELECT name FROM tenants WHERE id=? LIMIT 1');$q->execute([$tenantId]);return trim((string)($q->fetchColumn()?:''));}catch(Throwable){return'';}}
