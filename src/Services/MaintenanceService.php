...
final class MaintenanceService
{
    public function run():array
    {
        ...
        try{
            $stmt=$pdo->prepare('UPDATE api_refresh_tokens SET revoked_at=CURRENT_TIMESTAMP WHERE revoked_at IS NULL AND expires_at<CURRENT_TIMESTAMP');
            $stmt->execute();$result['expired_api_refresh_tokens']=$stmt->rowCount();
        }catch(\Throwable){$result['expired_api_refresh_tokens']=0;}
        ...
        try{
            if(Database::isSqlite($pdo))$pdo->exec("DELETE FROM api_refresh_tokens WHERE revoked_at IS NOT NULL AND revoked_at < datetime('now','-90 days')");
            else $pdo->exec('DELETE FROM api_refresh_tokens WHERE revoked_at IS NOT NULL AND revoked_at < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 90 DAY)');
            $result['old_api_refresh_tokens_removed']=true;
        }catch(\Throwable){$result['old_api_refresh_tokens_removed']=false;}
