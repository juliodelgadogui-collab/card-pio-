...
final class CashRegisterService
{
    ...
    public function open(int $openingBalanceCents=0,string $notes=''):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$userId,$openingBalanceCents,$notes){
            ...
            $s=$pdo->prepare('INSERT INTO cash_sessions (tenant_id,user_id,status,opening_balance_cents,opening_notes,opened_at) VALUES (?,? ,"open",?,?,NOW())');$s->execute([$tenantId,$userId,$openingBalanceCents,$this->note($notes)]);$id=(int)$pdo->lastInsertId();
            Auth::audit('cash.opened','cash_session',(string)$id,['opening_balance_cents'=>$openingBalanceCents]);
    ...
    {
        Auth::requirePermission('payments.manage');$tenantId=Auth::tenantId();$userId=Auth::id();if(!$tenantId||!$userId)return null;
        $s=Database::connection()->prepare('SELECT * FROM cash_sessions WHERE tenant_id=? AND user_id=? AND status="open" ORDER BY id DESC LIMIT 1');$s->execute([$tenantId,$userId]);$row=$s->fetch();return $row?$this->withTotals($row):null;
    }
    ...
    public function addMovement(string $type,int $amountCents,string $notes=''):array
    {
        ...
        return Database::transaction(function(PDO $pdo)use($tenantId,$userId,$type,$amountCents,$notes){
            $session=$this->lockCurrent($pdo,$tenantId,$userId);
            ...
            Auth::audit('cash.'.$type,'cash_session',(string)$session['id'],['amount_cents'=>$signed]);
    ...
    {
        Auth::requirePermission('payments.manage');$tenantId=Auth::tenantId();$userId=Auth::id();if(!$tenantId||!$userId)throw new RuntimeException('Sessão inválida.');$declaredCashCents=max(0,$declaredCashCents);
        return Database::transaction(function(PDO $pdo)use($tenantId,$userId,$declaredCashCents,$notes){
            $session=$this->lockCurrent($pdo,$tenantId,$userId);$expected=$this->expectedCash($pdo,$session);$difference=$declaredCashCents-$expected;
            ...
            Auth::audit('cash.closed','cash_session',(string)$session['id'],['expected_cash_cents'=>$expected,'declared_cash_cents'=>$declaredCashCents,'difference_cents'=>$difference]);
            return $this->getInTransaction($pdo,$tenantId,(int)$session['id']);
        });
    }
    ...
    public function recordManualRefund(PDO $pdo,int $tenantId,int $userId,int $orderId,int $amountCents,string $method,string $idempotencyKey):void
    {
        ...
        $session=$this->lockCurrent($pdo,$tenantId,$userId);$key='manual-refund:'.$idempotencyKey;
        $check=$pdo->prepare('SELECT id FROM cash_movements WHERE tenant_id=? AND idempotency_key=? LIMIT 1');$check->execute([$tenantId,$key]);if($check->fetchColumn())return;
    ...
    {
        Auth::requirePermission('payments.manage');$tenantId=Auth::tenantId();if(!$tenantId)return[];
        $s=Database::connection()->prepare('SELECT cm.*,u.name user_name FROM cash_movements cm LEFT JOIN users u ON u.id=cm.user_id WHERE cm.tenant_id=? AND cm.cash_session_id=? ORDER BY cm.id DESC LIMIT 300');$s->execute([$tenantId,$sessionId]);return $s->fetchAll();
    }

    private function lockCurrent(PDO $pdo,int $tenantId,int $userId):array
    {
        $s=$pdo->prepare('SELECT * FROM cash_sessions WHERE tenant_id=? AND user_id=? AND status="open" ORDER BY id DESC LIMIT 1 FOR UPDATE');$s->execute([$tenantId,$userId]);$row=$s->fetch();if(!$row)throw new RuntimeException('Abra o caixa antes de registrar pagamentos manuais.');return$row;
    }
