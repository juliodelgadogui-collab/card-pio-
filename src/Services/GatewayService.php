...
final class GatewayService
{
    ...
    public function save(string $provider, string $accountReference, array $config, string $webhookSecret, bool $active): void
    {
        ...
        $encrypted = Crypto::encrypt($config);
        $wh = $webhookSecret !== '' ? Crypto::encrypt($webhookSecret) : null;
        $pdo = Database::connection();
        $stmt = $pdo->prepare('INSERT INTO payment_gateways (tenant_id,provider,account_reference,config_encrypted,webhook_secret_encrypted,active) VALUES (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE account_reference=VALUES(account_reference),config_encrypted=VALUES(config_encrypted),webhook_secret_encrypted=COALESCE(VALUES(webhook_secret_encrypted),webhook_secret_encrypted),active=VALUES(active)');
        $stmt->execute([$tenantId,$provider,$accountReference,$encrypted,$wh,$active?1:0]);
    ...
    public function processWebhook(string $provider, string $tenantSlug, string $rawBody, array $headers, array $query): array
    {
        ...
        try {
            $ins=$pdo->prepare('INSERT INTO webhook_events (provider,external_event_id,signature_valid,payload_hash,status) VALUES (?,?,1,?,"received")');
            $ins->execute([$provider,$eventId,$hash]);
        ...
        try {
            if (!empty($verified['paid'])) {
                ...
                $pdo->prepare('UPDATE webhook_events SET status="processed",processed_at=NOW() WHERE provider=? AND external_event_id=?')->execute([$provider,$eventId]);
                return ['ok'=>true,'processed'=>true];
            }
            $pdo->prepare('UPDATE webhook_events SET status="ignored",processed_at=NOW() WHERE provider=? AND external_event_id=?')->execute([$provider,$eventId]);
            return ['ok'=>true,'processed'=>false];
        } catch (\Throwable $e) {
            $pdo->prepare('UPDATE webhook_events SET status="failed" WHERE provider=? AND external_event_id=?')->execute([$provider,$eventId]);
            throw $e;
        }
    }
    ...
    private function verifyStripe(array $gateway,array $config,string $secret,string $raw,array $headers): array
    {
        ...
        $key=(string)($config['secret_key']??''); if($key==='') throw new RuntimeException('Chave Stripe ausente.');
        ...
        $paid=((string)$pi->status==='succeeded');
        $orderId=(int)($pi->metadata->order_id??0);$tenantId=(int)($pi->metadata->tenant_id??0);
        if($tenantId!==(int)$gateway['tenant_id']||$orderId<1) throw new RuntimeException('Metadados Stripe inválidos.');
        return ['external_event_id'=>$eventId,'paid'=>$paid,'tenant_id'=>$tenantId,'order_id'=>$orderId,'provider'=>'stripe','provider_payment_id'=>(string)$pi->id,'amount_cents'=>(int)($pi->amount_received?:$pi->amount),'currency'=>strtoupper((string)$pi->currency),'account_reference'=>(string)$account->id,'raw_status'=>(string)$pi->status];
    }
    ...
    private function verifyPagBank(array $gateway,array $config,string $secret,string $raw,array $headers): array
    {
        ...
        $payload=json_decode($raw,true,512,JSON_THROW_ON_ERROR);$orderExternal=(string)($payload['id']??'');
        if($orderExternal==='') throw new RuntimeException('Pedido PagBank ausente.');
        $token=(string)($config['token']??'');if($token==='') throw new RuntimeException('Token PagBank ausente.');
        ...
        $paidCharge=null;foreach(($order['charges']??[]) as $charge){if(($charge['status']??'')==='PAID'){$paidCharge=$charge;break;}}
        $paid=$paidCharge!==null;
        $amount=$paid ? (int)($paidCharge['amount']['value']??0) : 0;
        return ['external_event_id'=>$orderExternal.':'.hash('sha256',$raw),'paid'=>$paid,'tenant_id'=>$tenantId,'order_id'=>$orderId,'provider'=>'pagbank','provider_payment_id'=>(string)($paidCharge['id']??$orderExternal),'amount_cents'=>$amount,'currency'=>'BRL','account_reference'=>(string)$gateway['account_reference'],'raw_status'=>(string)($paidCharge['status']??'')];
    }
    ...
    private function verifyMercadoPago(array $gateway,array $config,string $secret,string $raw,array $headers,array $query): array
    {
        ...
        $sig=$this->header($headers,'x-signature');$requestId=$this->header($headers,'x-request-id');
        ...
        $token=(string)($config['access_token']??'');if($token==='') throw new RuntimeException('Access token Mercado Pago ausente.');
    ...
    {
        $name=strtolower($name);foreach($headers as $k=>$v){if(strtolower((string)$k)===$name)return is_array($v)?(string)reset($v):(string)$v;}return '';
    }

    private function httpJson(string $method,string $url,array $headers=[],?array $body=null): array
    {
        $ch=curl_init($url);if($ch===false)throw new RuntimeException('Falha ao iniciar HTTP.');
        ...
        $data=json_decode((string)$response,true,512,JSON_THROW_ON_ERROR);if(!is_array($data))throw new RuntimeException('Resposta inválida do provedor.');return $data;
    }
}
