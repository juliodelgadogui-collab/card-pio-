...
final class MobileDeviceStatusService
{
    public function current(string $deviceIdentifier,int $tokenId): array
    {
        ...
        $nfc=$pdo->prepare('SELECT id,user_id,provider,name,status,pairing_attempts,paired_at,revoked_at,created_at FROM nfc_devices WHERE tenant_id=? AND device_identifier_hash=? LIMIT 1');
    ...
    public function requestNfcAuthorization(string $deviceIdentifier,int $tokenId): array
    {
        ...
        $deviceHash=hash('sha256',$deviceIdentifier);$deviceId=0;$requestChanged=false;$deviceName='EventMenu GO · '.mb_substr(Auth::name(),0,70);
        Database::transaction(function(PDO $tx)use($tenantId,$userId,$deviceHash,$deviceName,&$deviceId,&$requestChanged):void{
            $stmt=$tx->prepare(Database::portableSql($tx,'SELECT * FROM nfc_devices WHERE tenant_id=? AND device_identifier_hash=? FOR UPDATE'));
            ...
            if($device&&$device['status']==='pending'){
                if($device['user_id']!==null&&(int)$device['user_id']!==$userId)throw new RuntimeException('Este aparelho já possui uma solicitação pendente para outro funcionário.');
                $deviceId=(int)$device['id'];
                if($device['user_id']===null){
                    $tx->prepare('UPDATE nfc_devices SET user_id=?,name=? WHERE id=?')->execute([$userId,$device['name']?:$deviceName,$deviceId]);
                    $requestChanged=true;
                }
                return;
            }
            ...
            if($device){
                ...
                $deviceId=(int)$device['id'];$requestChanged=true;
            }else{
                ...
                $deviceId=(int)$tx->lastInsertId();$requestChanged=true;
            }
        });
        if($requestChanged)Auth::audit('nfc.authorization_requested','nfc_device',(string)$deviceId,['user_id'=>$userId]);
