...
use EventMenu\Services\DeliveryCustomerDocumentService;
...
function delivery_customer_error(Throwable $e): array{$m=trim($e->getMessage())?:'Não foi possível concluir agora. Tente novamente.';$code=match($m){'Confirme seu e-mail antes de entrar.'=>'EMAIL_NOT_VERIFIED','Sua sessão expirou. Entre novamente.','Entre na sua conta para continuar.'=>'UNAUTHENTICATED','E-mail ou senha inválidos.'=>'INVALID_CREDENTIALS','Já existe uma conta com este e-mail.'=>'EMAIL_IN_USE','Já existe uma conta com este CPF.'=>'CPF_IN_USE','Informe seu CPF para completar seu cadastro.','Informe seu CPF no perfil para continuar com o pagamento.'=>'CPF_REQUIRED',default=>'VALIDATION_ERROR'};$lower=mb_strtolower($m);foreach(['sqlstate','select ','insert ','update ','delete ','pdo','sqlite','mysql','database','stack trace','exception']as$t)if(str_contains($lower,$t))return ['code'=>'INTERNAL_ERROR','message'=>'Não foi possível concluir agora. Tente novamente.'];return ['code'=>$code,'message'=>mb_substr($m,0,260)];}
try{
$pdo=Database::connection();$action=strtolower(trim((string)($_GET['action']??'me')));$body=in_array($_SERVER['REQUEST_METHOD'],['POST','PUT','PATCH','DELETE'],true)?delivery_customer_body():[];$rate=new ApiRateLimitService();$auth=new DeliveryCustomerAuthService();$market=new DeliveryCustomerMarketplaceService();$payments=new DeliveryCustomerPaymentService();$subject=$rate->requestSubject('delivery-customer');
if($action==='register'){if($_SERVER['REQUEST_METHOD']!=='POST')delivery_customer_out(['ok'=>false,'message'=>'Ação indisponível.'],405);$rate->assertAllowed('delivery.customer.register',$subject,5,3600,'Muitas tentativas de cadastro. Aguarde e tente novamente.');delivery_customer_out(['ok'=>true]+$auth->register($pdo,$body),201);}
...
if($action==='payment-identification'){if($_SERVER['REQUEST_METHOD']!=='GET')delivery_customer_out(['ok'=>false,'message'=>'Ação indisponível.'],405);$rate->assertAllowed('delivery.customer.payment.identification',$rate->requestSubject('account:'.$accountId),20,300,'Muitas solicitações de pagamento. Aguarde um pouco.');$cpf=(new DeliveryCustomerDocumentService())->cpfFromAccount($account);if($cpf==='')throw new RuntimeException('Informe seu CPF no perfil para continuar com o pagamento.');delivery_customer_out(['ok'=>true,'identification'=>['type'=>'CPF','number'=>$cpf]]);}
...
if($action==='coupon-quote'){if($_SERVER['REQUEST_METHOD']!=='POST')delivery_customer_out(['ok'=>false,'message'=>'Ação indisponível.'],405);$rate->assertAllowed('delivery.customer.coupon',$rate->requestSubject('account:'.$accountId),30,300,'Muitas tentativas de cupom. Aguarde um pouco.');$tenantId=(int)($body['tenant_id']??0);$subtotal=max(0,(int)($body['subtotal_cents']??0));$code=(string)($body['code']??'');if($tenantId<1||$subtotal<1)throw new RuntimeException('Carrinho inválido para aplicar cupom.');delivery_customer_out(['ok'=>true,'coupon'=>$market->couponQuote($pdo,$accountId,$tenantId,$code,$subtotal)]);}
