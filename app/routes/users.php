...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='save'){
        ...
        try{
            Database::transaction(function(PDO $tx)use(&$id,$tenantId,$name,$email,$phone,$role,$status,$password,$selected):void{
                if($id){
            ...
            Auth::audit('user.saved','user',(string)$id,['role'=>$role,'status'=>$status,'effective_permissions'=>$selected]);
            em_flash('ok',$status==='blocked'?'Usuário bloqueado; app, NFC e turno foram encerrados.':'Usuário e permissões salvos.');
        }catch(Throwable $e){em_flash('error',$e->getMessage());}
        em_go('users');
    }
}

$editId=(int)($_GET['edit']??0);$edit=null;$selectedPermissions=[];
if($editId){
    $s=$pdo->prepare('SELECT * FROM users WHERE id=? AND tenant_id=?');$s->execute([$editId,$tenantId]);$edit=$s->fetch()?:null;
    if($edit)$selectedPermissions=PermissionCatalog::effectiveForUser($tenantId,(int)$edit['id'],(string)$edit['role']);
...
?><div class="grid" style="grid-template-columns:minmax(330px,1fr) minmax(0,2fr)"><section class="card"><h2><?= $edit?'Editar usuário':'Novo usuário' ?></h2><form method="post" class="form-grid" id="user-form"><input type="hidden" name="_csrf" value="<?= em_csrf() ?>"><input type="hidden" name="action" value="save"><input type="hidden" name="id" value="<?= (int)($edit['id']??0) ?>"><label class="span-2">Nome<input name="name" required value="<?= Security::e($edit['name']??'') ?>"></label><label class="span-2">E-mail<input type="email" name="email" required value="<?= Security::e($edit['email']??'') ?>"></label><label class="span-2">Telefone<input name="phone" value="<?= Security::e($edit['phone']??'') ?>"></label><label>Função-base<select name="role" id="user-role"><?php foreach($roles as $r):?><option value="<?= $r ?>"<?= em_selected($edit['role']??'waiter',$r) ?>><?= Security::e($roleLabels[$r]??$r) ?></option><?php endforeach;?></select></label><label>Status<select name="status"><option value="active"<?= em_selected($edit['status']??'active','active') ?>>Ativo</option><option value="blocked"<?= em_selected($edit['status']??'','blocked') ?>>Bloqueado</option></select></label><label class="span-2"><?= $edit?'Nova senha (deixe vazio para manter)':'Senha' ?><input type="password" name="password" minlength="10"<?= $edit?'':' required' ?>></label><div class="span-2"><h3>Permissões efetivas</h3><p class="muted">O cargo-base é apenas um preset. Marque exatamente o que este funcionário poderá fazer no painel e no EventMenu GO.</p><div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:8px"><?php foreach($permissionLabels as $permission=>$label):?><label class="checkbox"><input class="perm-check" type="checkbox" name="permissions[]" value="<?= Security::e($permission) ?>"<?= in_array($permission,$selectedPermissions,true)?' checked':'' ?>> <?= Security::e($label) ?></label><?php endforeach;?></div><button type="button" class="secondary" id="apply-role" style="margin-top:10px">Aplicar preset da função</button></div><button class="primary span-2">Salvar usuário</button><?php if($edit):?><a class="button secondary span-2" href="<?= Security::e(app_url('?route=users')) ?>">Cancelar</a><?php endif;?></form><p class="muted">Alterar uma permissão tem efeito no servidor imediatamente. O funcionário não consegue habilitar funções modificando o aplicativo.</p></section><section class="card"><div class="table-wrap"><table class="table"><thead><tr><th>Usuário</th><th>Função-base</th><th>Status</th><th>Turno</th><th>App</th><th>NFC</th><th></th></tr></thead><tbody><?php foreach($users as $u):?><tr><td><strong><?= Security::e($u['name']) ?></strong><br><span class="muted"><?= Security::e($u['email']) ?></span></td><td><span class="badge"><?= Security::e($roleLabels[$u['role']]??$u['role']) ?></span></td><td><?= Security::e($u['status']) ?></td><td><?= (int)$u['open_shifts']?'Aberto':'Fechado' ?></td><td><?= (int)$u['active_app_tokens'] ?></td><td><?= (int)$u['active_devices'] ?></td><td><a class="button secondary" href="<?= Security::e(app_url('?route=users&edit='.(int)$u['id'])) ?>">Editar</a></td></tr><?php endforeach;?></tbody></table></div></section></div>
