...
final class OrderCreationService
{
    public function create(array $payload):array
    {
        ...
        return Database::transaction(function(PDO $tx)use($tenantId,$userId,$channel,$tableId,$name,$phone,$address,$notes,$quantities):array{
            ...
            $i=$tx->prepare('INSERT INTO order_items (order_id,product_id,name_snapshot,unit_price_cents,quantity,total_cents) VALUES (?,?,?,?,?,?)');foreach($items as $item)$i->execute([$orderId,$item['id'],$item['name'],$item['price'],$item['qty'],$item['total']]);
