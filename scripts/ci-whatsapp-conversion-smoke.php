...
$clamped=$service->saveSettings($pdo,$tenantId,['repeat_last_order_enabled'=>1,'upsell_enabled'=>1,'upsell_max_suggestions'=>99,'abandoned_cart_enabled'=>1,'abandoned_delay_minutes'=>1]);wac_assert((int)$clamped['upsell_max_suggestions']===5&&(int)$clamped['abandoned_delay_minutes']===15,'Limites seguros das configurações de conversão não foram aplicados.');

$commerce=(string)file_get_contents(dirname(__DIR__).'/src/Services/WhatsAppCommerceService.php');$maintenance=(string)file_get_contents(dirname(__DIR__).'/src/Services/MaintenanceService.php');$route=(string)file_get_contents(dirname(__DIR__).'/app/routes/whatsapp.php');wac_assert(str_contains($commerce,'WhatsAppCommerceConversionService')&&str_contains($commerce,"state==='UPSELL'"),'Fluxo inbound não integrou repetir/upsell.');wac_assert(str_contains($maintenance,'recoverAbandonedCarts'),'Manutenção não integrou recuperação de carrinho.');wac_assert(str_contains($route,"action' value=\"save_conversion\"")||str_contains($route,'value="save_conversion"'),'Tela do WhatsApp não expõe salvamento das configurações de conversão.');wac_assert(str_contains($route,'repeat_last_order_enabled')&&str_contains($route,'upsell_max_suggestions')&&str_contains($route,'abandoned_delay_minutes'),'Tela do WhatsApp não expõe todos os controles de conversão.');wac_assert(str_contains($route,'Recursos para aumentar os pedidos')&&str_contains($route,'Salvar recursos de venda'),'Tela de conversão não usa linguagem amigável para o estabelecimento.');
