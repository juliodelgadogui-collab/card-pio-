...
final class PagarMePixProvider implements PixProviderInterface
{
    ...
    {
        $config=$gateway['config'];$key=trim((string)($config['secret_key']??$config['api_key']??''));$id=trim((string)($payment['provider_payment_id']??''));if($key===''||$id==='')throw new RuntimeException('Cobrança Pagar.me não está pronta para consulta.');$charge=$this->requestJson('Pagar.me','GET','https://api.pagar.me/core/v5/charges/'.rawurlencode($id),[],null,$key.':');$status=strtolower((string)($charge['status']??''));$amount=(int)($charge['paid_amount']??$charge['amount']??0);$currency=strtoupper((string)($charge['currency']??'BRL'));return['paid'=>$status==='paid','provider_payment_id'=>(string)($charge['id']??$id),'amount_cents'=>$amount,'currency'=>$currency,'account_reference'=>(string)$gateway['account_reference'],'raw_status'=>$status,'raw'=>$charge];
    }
}
